...
const JollyProducts = (() => {
  ...
  function renderCard(p) {
    ...
    const quickMenuBtn = !bulkSelectMode
      ...
      : '';
    // DÜZƏLİŞ (2026-07-25, #2): "İşarələ-sonra-Admin-təsdiqləsin" silmə axını.
    // Bu ✕ düyməsi məhsulu SİLMİR — yalnız "silinməli" siyahısına əlavə/çıxarır.
    // Əsl silmə yalnız Admin-in "İşarəli Məhsullar" ekranından edilir.
    const isMarked = (typeof JollyDB.isMarkedForDeletion === 'function') && JollyDB.isMarkedForDeletion(p.id);
    const markBtn = !bulkSelectMode
      ? `<span onclick="event.stopPropagation();JollyProducts.toggleMarkForDeletion('${p.id}')" title="${isMarked ? 'Silinmə işarəsini geri al' : 'Silinmək üçün işarələ'}" style="position:absolute;top:6px;left:6px;z-index:6;width:26px;height:26px;border-radius:50%;background:${isMarked ? 'var(--accent-danger,#ff5c6c)' : 'rgba(0,0,0,0.45)'};backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;color:#fff;font-size:14px;cursor:pointer;">✕</span>`
      : '';
    const cardClick = bulkSelectMode ? `JollyProducts.toggleBulkSelect('${p.id}')` : `JollyRouter.go('#/product/${p.id}')`;
    return `
      <div class="glass product-card ${glowClass}" data-id="${p.id}" onclick="${cardClick}" style="position:relative;${gColor ? `border-left:4px solid ${gColor};` : ''}${isSelected ? 'border-color:var(--accent-1,#7c8aff);box-shadow:0 0 0 2px var(--accent-1,#7c8aff);' : ''}${isMarked ? 'opacity:0.55;border:1.5px dashed var(--accent-danger,#ff5c6c);' : ''}">
        ${bulkCheck}
        ${markBtn}
        ${quickMenuBtn}
  ...
  function _currentActorName() {
    try {
      ...
      if (!s) return 'Admin';
      return s.role === 'admin' ? 'Admin' : (s.userName || 'İşçi');
  ...
  function toggleMarkForDeletion(id) {
    const already = JollyDB.isMarkedForDeletion(id);
    if (already) {
      if (!_isAdminSession()) {
        Toast.error('🔒 Silinmə işarəsini yalnız Admin geri ala bilər');
        return;
      }
      JollyDB.unmarkForDeletion(id);
      Toast.success('İşarə geri alındı');
    } else {
      JollyDB.markForDeletion(id, _currentActorName());
      if (typeof JollySound !== 'undefined') JollySound.warn();
      Toast.info('✕ Silinmək üçün işarələndi — Admin təsdiqləməlidir');
    }
    _refreshCurrentProductList();
  }

  // Admin-only ekran — bütün işarələnmiş məhsulları göstərir, yalnış
  // işarələməni geri almağa VƏ əsl silməni (Trash-ə köçürərək) təsdiqləməyə
  // imkan verir. Adi işçi bu ekrana girə bilmir.
  function renderMarkedForDeletionPage() {
    if (!_isAdminSession()) {
      if (window.JollyRouter) setTimeout(() => JollyRouter.go('#/home'), 0);
      return `<div class="empty-state"><div class="big-icon">🔒</div><h3>İcazə yoxdur</h3><p class="muted" style="font-size:12px;">Bu ekranı yalnız Admin görə bilər.</p></div>`;
    }
    const marks = JollyDB.getMarkedForDeletion();
    return `
      <h2 style="font-family:var(--font-display);margin:0 0 4px;font-size:19px;">✕ İşarəli Məhsullar</h2>
      <p class="muted" style="font-size:12px;margin:0 0 14px;">İşçilər tərəfindən "silinməli" işarəsi qoyulmuş məhsullar — yalnız sən təsdiqləyə bilərsən.</p>
      <div id="markedListZone">${_renderMarkedList(marks)}</div>
    `;
  }

  function _renderMarkedList(marks) {
    if (!marks.length) return '<div class="empty-state"><div class="big-icon">✅</div><h3>İşarəli məhsul yoxdur</h3></div>';
    return marks.map(m => {
      const p = JollyDB.Products.get(m.productId);
      if (!p) return ''; // məhsul artıq başqa yolla silinib
      const when = new Date(m.markedAt).toLocaleString('az-AZ');
      return `
        <div class="glass" style="padding:12px 14px;margin-bottom:10px;display:flex;align-items:center;gap:12px;">
          <div class="thumb" style="width:56px;height:56px;flex-shrink:0;border-radius:10px;overflow:hidden;">${(p.images && p.images[0]) ? `<img ${typeof JollyStorage !== 'undefined' ? JollyStorage.imgAttr(p.images[0]) : 'src="' + p.images[0] + '"'} style="width:100%;height:100%;object-fit:cover;">` : '🧴'}</div>
          <div style="flex:1;min-width:0;">
            <div style="font-weight:600;font-size:14px;">${escapeHtml(p.name || 'Adsız məhsul')}</div>
            <div class="muted" style="font-size:11px;">✕ ${escapeHtml(m.markedBy || 'Naməlum')} · ${when}</div>
          </div>
          <button class="btn btn-ghost btn-sm" onclick="JollyProducts.unmarkFromReview('${m.productId}')">↩️ Geri al</button>
          <button class="btn btn-danger btn-sm" onclick="JollyProducts.confirmMarkedDelete('${m.productId}')">🗑️ Sil</button>
        </div>
      `;
    }).join('');
  }

  function unmarkFromReview(productId) {
    JollyDB.unmarkForDeletion(productId);
    Toast.success('İşarə geri alındı');
    const zone = document.getElementById('markedListZone');
    if (zone) zone.innerHTML = _renderMarkedList(JollyDB.getMarkedForDeletion());
  }

  function confirmMarkedDelete(productId) {
    const p = JollyDB.Products.get(productId);
    const name = (p && p.name) ? p.name : 'Məhsul';
    if (!confirm(`"${name}" həqiqətən silinsin? (Silinənlər səbətinə düşəcək, 30 gün ərzində bərpa edə bilərsən)`)) return;
    JollyDB.Trash.moveToTrash(productId);
    JollyDB.unmarkForDeletion(productId);
    if (typeof JollySound !== 'undefined') JollySound.warn();
    if (window.JollyEvents) JollyEvents.emit('product.deleted', { id: productId });
  ...
  function renderHomePage() {
    ...
    return `
      ...
      <div class="chip-row" style="margin-bottom:6px;" id="homeFilterChips">
        ...
        ${_isAdminSession() && JollyDB.getMarkedForDeletion().length ? `<span class="chip" style="border-color:var(--accent-danger,#ff5c6c);color:var(--accent-danger,#ff5c6c);" onclick="JollyRouter.go('#/marked-for-deletion')">✕ İşarəli (${JollyDB.getMarkedForDeletion().length})</span>` : ''}
      </div>
  ...
  function refreshBulkUI() {
    ...
    if (bulkSelectMode && bulkSelectedIds.size > 0) {
      // DÜZƏLİŞ (2026-07-23): "🗑️ Sil" toplu silmə düyməsi əlavə olundu —
      // əvvəllər bu panel yalnız köçürmə+paylaşım edirdi, silmə heç yerdə
      // yox idi. Həm bu, həm də "Qrupa köçür" indi müvafiq icazəyə
      // (products.delete / products.edit) bağlıdır ki, Zülfiqar kimi
      // işçilər üçün İcazə Mərkəzindən ayrıca aç/bağla edilə bilsin.
      const canDelete = !(window.JollyAuth && !JollyAuth.can('products.delete'));
      const html = `
  ...
  function bulkDeleteSelected() {
    ...
    if (!confirm(`Seçilmiş ${count} məhsul silinsin? (Silinənlər səbətinə düşəcək, 30 gün ərzində bərpa edə bilərsən)`)) return;
    if (!checkPinForDelete()) return;
    const ids = [...bulkSelectedIds];
    ids.forEach(id => { JollyDB.Trash.moveToTrash(id); });
    if (typeof JollySound !== 'undefined') JollySound.warn();
    if (window.JollyEvents) ids.forEach(id => JollyEvents.emit('product.deleted', { id }));
    bulkSelectedIds.clear();
    bulkSelectMode = false;
    Toast.success(`${count} məhsul silindi`);
    showUndoSnackbar(`${count} məhsul silindi`, () => {
      ids.forEach(id => JollyDB.Trash.restore(id));
      if (typeof JollySound !== 'undefined') JollySound.success();
  ...
  function renderDetailPage(id) {
    ...
    return `
      <div class="glass" style="padding:16px;">
        ...
        ${p.alertFlag ? `<div style="background:rgba(255,77,77,0.15);border-left:3px solid var(--accent-danger,#ff5c6c);padding:12px;border-radius:4px;margin-bottom:14px;color:var(--accent-danger,#ff5c6c);font-size:13px;font-weight:600;"><strong>⚠️ Xəbərdarlıq</strong><br>Bu məhsul diqqət tələb edir — aşağıdakı qeydə bax.</div>` : ''}

        ${p.sectionNote ? `<div style="background:rgba(251,191,36,0.12);border-left:3px solid var(--accent-warn,#fbbf24);padding:12px;border-radius:4px;margin-bottom:14px;color:var(--accent-2,#fff);font-size:13px;font-style:italic;"><strong>📝 Bölmə Qeydyatı:</strong><br>${escapeHtml(p.sectionNote)}</div>` : ''}
  ...
  function renderFormPage(id, draftId) {
    ...
    return `
      ...
      <div class="glass" style="padding:16px;">
        ...
        <div class="field">
          <label>🚨 Xəbərdarlıq Flags</label>
          <div style="display:flex;flex-direction:column;gap:8px;">
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
              <input type="checkbox" id="f_alertFlag" ${formState.alertFlag ? 'checked' : ''} style="cursor:pointer;">
              <span>Bu məhsul diqqət tələb edir (🚨 badge göstər)</span>
            </label>
          </div>
        </div>

        <div class="field">
          <label>📝 Bölmə Qeydyatı</label>
          <textarea id="f_sectionNote" placeholder="Məsələn: 'Bu unika qırmızıdır amma kodu Huda Sampun vurur — diqqətli ol!'" style="min-height:80px;">${escapeHtml(formState.sectionNote || '')}</textarea>
        </div>
  ...
  function afterFormRender() {
    renderImageStrip();
    renderBarcodeTags();
    ['name', 'mainCode', 'price', 'extraCodeType', 'extraCodeValue', 'color', 'note', 'sectionNote', 'last4', 'expiryDate'].forEach(f => {
      const el = document.getElementById('f_' + f);
      if (el) {
    ...
    const alertFlagEl = document.getElementById('f_alertFlag');
    if (alertFlagEl) {
      alertFlagEl.addEventListener('change', () => { formState.alertFlag = alertFlagEl.checked; });
    }
    const nameEl = document.getElementById('f_name');
  ...
  function submitForm(keepOpen) {
    ...
    // Barkod duplikat check (2026-07-26, #3)
    if (formState.barcodes && formState.barcodes.length > 0) {
      for (let barcode of formState.barcodes) {
        const conflict = JollyDB.Products.checkBarcodeConflict(barcode, formState.id);
        if (conflict) {
          if (!confirm(`🚨 Kod duplikatı!\n\nBu kod başqa məhsulda var: "${conflict.name}"\n\n${formState.name} üçün ayrı kod qur, yoxsa yenə də saxlanılsın?`)) {
            return false;
          }
        }
      }
  ...
  return {
    ...
    toggleMarkForDeletion, renderMarkedForDeletionPage, unmarkFromReview, confirmMarkedDelete,
  };
})();
