...
const JollyProducts = (() => {
  ...
  // ── Qrup daxili axtarış + qalereya görünüşü (2026-07-22) ──
  // Qrup böyük olanda (məs. "açkı 545" — 1000 çeşid), sadə siyahı
  // əvəzinə axtarış qutusu + istəyə görə şəkil-əsaslı qalereya grid-i.
  ...
  function renderFilteredPage(params) {
    ...
    return `
      ...
      <div class="row" style="gap:8px;margin-bottom:12px;">
        ...
        <button class="icon-btn" id="galleryToggleBtn" onclick="JollyProducts.toggleGroupView()" title="Qalereya görünüşü">🖼️</button>
      </div>` : ''}
  ...
  function _renderGroupFilteredList() {
    ...
    if (_groupViewState.mode === 'gallery') {
      const cells = [];
      list.forEach(p => (p.images || []).forEach((img, i) => cells.push({ img, name: p.name, images: p.images, idx: i, id: p.id, route: `#/product/${p.id}` })));
      el.innerHTML = cells.length ? `<div class="gallery-grid">
        ${cells.map(c => `
          <div class="gallery-cell anim-pop peekable" style="position:relative;" onclick='JollyDashboard.openGalleryImage(${escapeHtml(JSON.stringify(c.images))}, ${c.idx})' oncontextmenu="event.preventDefault();JollyRouter.go('${c.route}')">
            <span onclick="event.stopPropagation();JollyProducts.deleteFromGroupView('${c.id}')" title="Sil" style="position:absolute;top:5px;right:5px;z-index:5;width:26px;height:26px;border-radius:50%;background:rgba(0,0,0,0.55);backdrop-filter:blur(4px);color:#fff;display:flex;align-items:center;justify-content:center;font-size:13px;border:1px solid rgba(255,255,255,0.15);">🗑️</span>
        ...
        `).join('')}
      </div>` : `<div class="empty-state"><div class="big-icon">🖼️</div><h3>Şəkil yoxdur</h3></div>`;
    } else {
      renderList(el, list);
    }
  }
  ...
  function toggleGroupView() {
    ...
    if (btn) btn.textContent = _groupViewState.mode === 'gallery' ? '📋' : '🖼️';
    _renderGroupFilteredList();
  }
  ...
  // Qrup ekranından (qalereya/siyahı) silmə — adi deleteProduct()-dan
  // fərqli olaraq #/home-a atmır, elə həmin qrup ekranında qalır ki,
  // bir neçəsini ard-arda silmək lazım olanda hər dəfə yenidən qrupa
  // girmək lazım gəlməsin.
  function deleteFromGroupView(id) {
    ...
    _groupViewState.all = _groupViewState.all.filter(x => x.id !== id);
    _renderGroupFilteredList();
    showUndoSnackbar(`"${escapeHtml(name)}" silindi`, () => {
      ...
      if (restored) _groupViewState.all.push(restored);
      _renderGroupFilteredList();
    });
  }
  ...
  return {
    ...
    filterGroupSearch, toggleGroupView, deleteFromGroupView,
