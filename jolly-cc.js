...
(function (global) {
  ...
  function field(label, val, mono) {
    ...
    return '<div class="cc-kv"><span>' + esc(label) + '</span>' +
      '<b class="cc-cp' + (mono ? ' mono' : '') + '" draggable="true" ' +
        'data-v="' + esc(v) + '" ' +
        'onclick="JollyCC.grab(this)" ' +
        'ondragstart="JollyCC.drag(event,this)" ' +
        'title="toxun — kopyalanır · sürüklə — 1C-yə apar">' +
        esc(v) + '<i class="cc-cpi">⧉</i></b></div>';
  }
  ...
  function showCard(p) {
    ...
    var html =
      '<div class="cc-card cc-detail">' +
        ...
        '<div class="cc-title cc-cp" draggable="true" data-v="' + esc(p.name || '') + '" ' +
          'onclick="JollyCC.grab(this)" ondragstart="JollyCC.drag(event,this)">' +
          esc(p.name || 'Adsız') + '</div>' +
        (p.price ? '<div class="cc-big cc-cp" draggable="true" data-v="' + esc(p.price) + '" ' +
          'onclick="JollyCC.grab(this)" ondragstart="JollyCC.drag(event,this)">' +
          p.price + ' <small>₼</small></div>' : '') +
        ...
        (enc.ok ? '<div class="cc-bc">' + BC.svg(enc.bits) +
          '<div class="cc-bcn cc-cp" draggable="true" data-v="' + esc(bc) + '" ' +
            'onclick="JollyCC.grab(this)" ondragstart="JollyCC.drag(event,this)">' +
            esc(bc) + '</div></div>' : '') +
  ...
  function render() {
    ...
    return '<div class="storeos cc-wrap">' +
      ...
      '<div class="cc-split">' +
        ...
        '<div class="cc-right"><div id="ccSide">' + (side ||
          '<div class="cc-empty">Mala toxun —<br>kartı burada açılacaq</div>') + '</div></div>' +
      '</div>' +
      '</div>';
  }
  ...
  setInterval(function () { ensureScreen(); keepFocus(); }, 900);
