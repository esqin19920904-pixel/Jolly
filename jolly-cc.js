...
(function (global) {
  ...
  function histHtml() {
    ...
    return '<div class="cc-sec">Son baxılanlar</div><div class="cc-hist">' +
      l.map(function (x) {
        return '<div class="cc-hrow" onclick="JollyCC.open(\'' + x.id + '\')">' +
          '<span class="n">' + esc(String(x.name).slice(0, 28)) + '</span>' +
          (x.price ? '<b>' + x.price + ' ₼</b>' : '') +
          (x.bc ? '<i>' + esc(String(x.bc).slice(-6)) + '</i>' : '') +
        '</div>';
      }).join('') + '</div>' +
      ...
      '<div class="cc-card cc-detail">' +
        ...
        (p.price
          ? '<div class="cc-big' + (bigPrice ? ' huge' : '') + '">' + p.price +
            ' <small>₼</small></div>'
          : '<div class="cc-big nop">qiymətsiz</div>') +
