...
(function (global) {
  ...
  function doImgMove(start) {
    ...
    if (!C2 || !C2.migrateImages) {
      paint('<div class="cc-card cc-warn"><div class="cc-h">Bulud körpüsü yüklənməyib</div>' +
        '<div class="cc-p">jolly-cloud2.js yoxdur.</div></div>');
      return;
    }

    var c = C2.localCount();
    if (!c.images) {
      paint('<div class="cc-card cc-ok"><div class="cc-h">✅ Hamısı serverdədir</div>' +
        '<div class="cc-p">Cihazda qalan şəkil yoxdur — hər cihazda görünür.</div></div>');
      return;
    }
    ...
    if (!start) {
      paint('<div class="cc-card cc-warn">' +
        ...
        '<div class="cc-p">' + c.products + ' malın şəkli yalnız bu telefonda saxlanılıb — ' +
  ...
  function css() {
    ...
    st.textContent = [
      ...
      '@media (min-width:900px){',
        ...
        '.cc-grid{grid-template-columns:repeat(auto-fill,minmax(190px,1fr))}',
        '.cc-gal{grid-template-columns:repeat(auto-fill,minmax(200px,1fr))}',
      ...
      '@media (min-width:1500px){',
        ...
        '.cc-grid{grid-template-columns:repeat(auto-fill,minmax(210px,1fr))}',
      '}',
  ...
  var HIDE = [
    ...
    '.rfab-main', '.rfab-petals', '.rfab-favs',   /* yaşıl ＋ dairəvi menyu */
    '.qfab-main', '.qfab-menu',           /* sarı lupa */
    '.edge-tab', '.edge-panel', '.edge-scrim',    /* yan panel */
    '.backup-dot', '.icon-btn'
  ];
  ...
  var LIGHT = [
    /* ── fon və mətn ── */
    'body,.storeos{background:#f4f6fa !important;color:#1a1f2e !important}',
    '.cc-wrap{color:#1a1f2e}',
    '.muted,.cc-hint,.cc-p,.cc-note{color:#5b6478 !important;opacity:1 !important}',
    '.cc-sec{color:#8a93a6 !important;opacity:1 !important}',

    /* ── xana və düymələr ── */
    '.cc-in{background:#fff !important;color:#1a1f2e !important;',
    'border:1.5px solid #d3d9e4 !important;box-shadow:none !important}',
    '.cc-in:focus{border-color:#2563eb !important;background:#fff !important}',
    '.cc-in::placeholder{color:#9aa3b5 !important}',
    '.cc-ic{background:#fff !important;color:#1a1f2e !important;',
    'border:1.5px solid #d3d9e4 !important;box-shadow:none !important}',
    '.cc-ic.cc-go{background:#2563eb !important;color:#fff !important;border:none !important}',
    '.cc-btn{background:#fff !important;color:#1a1f2e !important;',
    'border:1.5px solid #d3d9e4 !important}',
    '.cc-btn.cc-pri{background:#2563eb !important;color:#fff !important;border:none !important}',
    '.cc-act{background:#f0f3f8 !important;color:#1a1f2e !important;',
    'border:1.5px solid #d9dfe9 !important}',

    /* ── bölmə zolağı ── */
    '.cc-tab{background:#fff !important;color:#4a5468 !important;',
    'border:1.5px solid #d3d9e4 !important}',
    '.cc-tab.on{background:#e8efff !important;color:#1d4ed8 !important;',
    'border-color:#2563eb !important}',

    /* ── kartlar və kafellər ── */
    '.cc-card,.cc-cell,.cc-gcell,.cc-stat,.cc-spec,.cc-folder,.cc-hist{',
    'background:#fff !important;border:1.5px solid #e2e7f0 !important;',
    'box-shadow:none !important}',
    '.cc-cell img,.cc-ph,.cc-gcell img{background:#f0f3f8 !important}',
    '.cc-nm,.cc-title{color:#1a1f2e !important}',
    '.cc-hrow{background:#fff !important;border-bottom:1px solid #eef1f6 !important;',
    'color:#1a1f2e !important}',
    '.cc-kv,.cc-spec .v{color:#1a1f2e !important}',
    '.cc-spec .l,.cc-kv span{color:#7b8497 !important;opacity:1 !important}',

    /* ── ★ RƏNGLƏR SAKİTLƏŞİR: yalnız qiymət və xəbərdarlıq ── */
    '.cc-price,.cc-py{color:#0f766e !important;text-shadow:none !important}',
    '.cc-chip{background:#eef2ff !important;color:#3730a3 !important;',
    'border:1px solid #c7d2fe !important}',
    '.cc-chip.new{background:#dcfce7 !important;color:#166534 !important;',
    'border-color:#86efac !important}',
    '.cc-chip.soft{background:#f1f5f9 !important;color:#475569 !important;',
    'border-color:#e2e8f0 !important}',
    '.cc-newtag{background:#16a34a !important;color:#fff !important}',
    '.cc-pill{background:#fff !important;color:#334155 !important;',
    'border:1.5px solid #d3d9e4 !important}',
    '.cc-stat b{color:#1e40af !important}',
    '.cc-bcx,.cc-fcode2{color:#94a3b8 !important;opacity:1 !important}',

    /* ── xəbərdarlıqlar (yeganə güclü rəng) ── */
    ...
    '.cc-card.cc-ai{background:#eff6ff !important;border-color:#93c5fd !important}',
    '.cc-alert{background:#fef2f2 !important;border:2px solid #fca5a5 !important}',
    '.cc-atitle{color:#b91c1c !important}',
    ...
    '.cc-gaps{background:#fef3c7 !important;color:#92400e !important;',
    'border-color:#fcd34d !important}',
    '.cc-miss{background:rgba(0,0,0,.62) !important;color:#fff !important}',

    /* ── barkod düyməsi — kassirin əsas aləti ── */
    '.cc-copybtn{background:#ecfdf5 !important;color:#065f46 !important;',
    'border:2px solid #6ee7b7 !important}',
    '.cc-copybtn.cc-copied{background:#a7f3d0 !important}',
    '.cc-copybtn .lab{color:#047857 !important;opacity:1 !important}',
    '.cc-empty{border-color:#d3d9e4 !important;color:#94a3b8 !important;opacity:1 !important}',
    '.cc-note-box{background:#f8fafc !important;color:#334155 !important;',
    'border-color:#e2e8f0 !important}',
    '.cc-undo{background:#eff6ff !important;border-color:#93c5fd !important;',
    'color:#1e3a8a !important}',

    /* ── ★ ANİMASİYA SIFIR — zəif kompüterdə yüngül işləsin ── */
  ...
  setInterval(function () { guardWorker(); ensureScreen(); keepFocus(); }, 900);
