...
(function (global) {
    ...
    var html =
      '<div class="cc-card cc-detail">' +
        ...
        (suspectPrice(p)
          ...
          : '<div class="cc-big' + (bigPrice ? ' huge' : '') + '">' + p.price +
            ' <small>₼</small></div>') +
        (gaps.length ? '<div class="cc-gaps">⚠️ ' + gaps.join(' · ') + '</div>' : '') +

        '<div class="cc-kvs">' +
          field('Barkod', (p.barcodes || []).join(', '), true) +
          field('Firma', p.brand) +
          field('Qrup', p.group) +
  ...
  var IMG_DEFAULT = 'https://kodsuz-mehsullar.pages.dev/api/jolly-img';
  ...
  function addPhoto(id) {
    ...
    inp.onchange = function () {
      ...
      /* 1) Əvvəl serverə — orada itmir və yer tutmur */
      uploadToCloud(f).then(function (url) {
  ...
  function css() {
    ...
    st.textContent = [
      ...
      '@media (min-width:900px){',
        '.cc-wrap{max-width:1400px;margin:0 auto}',
        '.cc-in{font-size:17px;padding:17px 18px}',
        '.cc-ic{width:54px;height:54px}',
        '.cc-split{display:grid;grid-template-columns:1fr 420px;gap:18px;align-items:start}',
        '.cc-right{position:sticky;top:14px}',
        '.cc-grid{grid-template-columns:repeat(4,1fr)}',
        '.cc-gal{grid-template-columns:repeat(4,1fr)}',
      ...
      '<div class="cc-hint">' +
        (isWorker()
          ? '<b>545</b> → barkodla tap · <b>corab</b> → adla tap · ' +
            '<b>F2</b> xana · <b>F3</b> son mal · <b>F4</b> barkodu kopyala'
  ...
  function takeCommand() {
    ...
    /* 1 — köhnə işçi iş masasını söndür */
    try {
      var raw = localStorage.getItem('jolly_user_mode');
      var cfg = raw ? JSON.parse(raw) : {};
      if (cfg && (cfg.on !== false || cfg.pressDebug)) {
        cfg.on = false;
        cfg.whitelist = false;
        cfg.permDriven = false;
        cfg.pressDebug = false;
        localStorage.setItem('jolly_user_mode', JSON.stringify(cfg));
      }
    } catch (e) {}

    /* 1b — köhnə modul öz obyektini də çəkməsin */
    try {
  ...
  document.addEventListener('keydown', function (e) {
    try {
      ...
      /* ★ Kassa üçün — siçana toxunmadan işləmək */
      if (String(e.key) === 'F2') {          /* xanaya keç və təmizlə */
        e.preventDefault();
        if (!onRoute()) go(ROUTE);
        setInput(''); focusInput();
        return;
      ...
      if (String(e.key) === 'F4' && onRoute()) {   /* barkodu kopyala */
        e.preventDefault();
        if (ctxProduct && (ctxProduct.barcodes || [])[0]) {
          var fake = { getAttribute: function () { return ctxProduct.barcodes[0]; },
                       classList: { add: function () {}, remove: function () {} } };
          grab(fake);
        }
        return;
