...
(function (global) {
  ...
  function detect(raw) {
    ...
    var nt = norm(t);
    if (/^cixis$|^çıxış$|^admin$|^giris$|^giriş$|^hesab$/.test(nt)) {
      return { kind: 'exit' };
    }
    if (/^bildiri[şs]|^tapilmayan|^tap[ıi]lmayan|^naməlum|^namelum|^tanimayan/.test(nt)) {
      return { kind: 'unknown' };
    }
  ...
  function submit(raw) {
    ...
    if (it.kind === 'exit') return doExit();
    if (it.kind === 'unknown') return doUnknown();
    ...
    (wide() ? paintSide : paint)(
      '<div class="cc-alert' + (bad ? ' bad' : '') + '">' +
        ...
        '<div class="cc-row" style="margin-top:14px">' +
          ...
          '<button class="cc-btn cc-pri" onclick="JollyCC.report(\'' + esc(code) + '\')">📮 Adminə bildir</button>' +
          (isWorker() ? ''
  ...
  function diag() {
    ...
    var rows = [
      ...
      ['🔒 Çıxış qoruması', appPin() ? 'proqramın Admin PIN-i'
        : (ccPin() ? 'kassa PIN-i' : '⚠️ YOXDUR — hər kəs çıxa bilər')]
    ];
  ...
  /* ══════════════════════════════════════════════════════════
     🔑 ÇIXIŞ — kassirdən adminə qayıtmaq
     ──────────────────────────────────────────────────────────
     Esqin: "admin/kassir keçidi bağladın, bəs necə keçəcəyik?"

     HAQLI İDİ — qabığı gizlədəndə hesab dəyişmə düyməsi də
     gizlənmişdi. Çıxış yolu OLMADAN kilid qurmaq səhvdir.

     İndi iki yol var, ikisi də proqramın öz girişini işlədir
     (`JollySecurity.switchUser` — şifrə orada soruşulur):
       · xanaya `çıxış` və ya `admin` yazmaq
       · ekranın SAĞ AŞAĞI küncündəki kiçik 🔑 düyməsi

     Kilid açarı DEYİL — sadəcə giriş ekranını açır. Kassir şifrəni
     bilmirsə admin ola bilmir.
     ══════════════════════════════════════════════════════════ */
  /* ── Kassa PIN-i ─────────────────────────────────────────
     Esqin: "kodsuz adminə keçdi axı".

     SƏBƏB: `security.js`-də `pinHash: null` — proqramın Admin PIN-i
     HEÇ VAXT TƏYİN EDİLMƏYİB, ona görə hesab dəyişəndə şifrə
     soruşulmur. Bu, çıxış düyməsindən əvvəl də belə idi.

     İki qat qoruma:
       1. Proqramın öz Admin PIN-i varsa — o işləyir (ən düzgünü,
          bütün proqramı qoruyur)
       2. Yoxdursa — Command Center-in öz PIN-i soruşulur
          (`jolly_cc_pin`). Admin onu bir dəfə təyin edir
       3. İkisi də yoxdursa — çıxış verilir, AMMA açıq xəbərdarlıq
          olunur ki, qoruma yoxdur
     ─────────────────────────────────────────────────────── */
  var PIN_KEY = 'jolly_cc_pin';
  ...
  function appPin() {
    try {
      var S = G('JollySecurity');
      var cfg = (S && S.getCfg) ? S.getCfg() : null;
      return (cfg && cfg.pinHash) ? String(cfg.pinHash) : '';
    } catch (e) { return ''; }
  }

  function doExit() {
    /* 1 — proqramın öz PIN-i varsa ona həvalə edirik */
    var S = G('JollySecurity');
    if (appPin() && S && S.switchUser) {
      try { S.switchUser(); return; } catch (e) {}
    }

    /* 2 — bizim PIN varsa soruşuruq */
    if (ccPin()) {
      paint('<div class="cc-card cc-warn">' +
        '<div class="cc-h">🔑 Admin PIN</div>' +
        '<div class="cc-f"><span>PIN</span>' +
          '<input id="ccPinIn" type="password" inputmode="numeric" maxlength="8" ' +
          'placeholder="••••" onkeydown="if(event.key===\'Enter\')JollyCC.pinOk()"></div>' +
        '<div class="cc-row">' +
          '<button class="cc-btn cc-pri" onclick="JollyCC.pinOk()">Aç</button>' +
          '<button class="cc-btn" onclick="JollyCC.cancel()">Ləğv et</button>' +
        '</div></div>');
      setTimeout(function () {
        var el = document.getElementById('ccPinIn');
        if (el) { try { el.focus(); } catch (e) {} }
      }, 80);
      return;
    }

    /* 3 — heç bir qoruma yoxdur: açıq deyirik */
    paint('<div class="cc-card cc-warn">' +
      '<div class="cc-h">⚠️ Qoruma qurulmayıb</div>' +
      '<div class="cc-p">Admin PIN təyin edilməyib — indi <b>kim olursa olsun</b> ' +
      'admin rejiminə keçə bilər. Kassada bu təhlükəlidir.<br><br>' +
      'Aşağıdan PIN qoy: yalnız onu bilən çıxa biləcək.</div>' +
      '<div class="cc-f"><span>Yeni PIN</span>' +
        '<input id="ccPinNew" type="password" inputmode="numeric" maxlength="8" ' +
        'placeholder="4–8 rəqəm"></div>' +
      '<div class="cc-row">' +
        '<button class="cc-btn cc-pri" onclick="JollyCC.pinSet()">🔒 PIN qoy və çıx</button>' +
        '<button class="cc-btn" onclick="JollyCC.exitNow()">PİNSİZ çıx</button>' +
      '</div></div>');
    return;
  }
  ...
  function exitNow() {
    var S = G('JollySecurity');
    if (S && S.switchUser) { try { S.switchUser(); return; } catch (e) {} }
    /* Təhlükəsizlik modulu yoxdursa sessiyanı təmizləyib açılışa */
    paint('<div class="cc-card cc-warn">' +
      '<div class="cc-h">🔑 Hesabdan çıxış</div>' +
      '<div class="cc-p">Giriş modulu tapılmadı. Aşağıdakı düymə sessiyanı ' +
      'bağlayıb proqramı yenidən açacaq.</div>' +
      '<div class="cc-row">' +
        '<button class="cc-btn cc-pri" onclick="JollyCC.exitHard()">Çıx</button>' +
        '<button class="cc-btn" onclick="JollyCC.cancel()">Ləğv et</button>' +
      '</div></div>');
  }

  /* ══════════════════════════════════════════════════════════
     📮 TANINMAYAN KODLAR — kassirdən adminə
     ──────────────────────────────────────────────────────────
     Esqin: "elavə bir mod qoyaq — məhsul tapılmadı, bir xana.
     Kassirlər olur ki, məhsullar çıxır, sistem tanımır — o kodu
     ora vursunlar, sonra biz adminlər baxa bilək, düzəldək."

     Yəni kassa dayanmır: kod bir xanaya yazılır, iş davam edir.
     Admin sonra siyahıya baxıb malı yaradır.
     ══════════════════════════════════════════════════════════ */
  var UNK_KEY = 'jolly_cc_unknown';

  function unkList() {
    try { return JSON.parse(localStorage.getItem(UNK_KEY) || '[]'); } catch (e) { return []; }
  }
  function unkSave(l) {
    try { localStorage.setItem(UNK_KEY, JSON.stringify(l.slice(0, 300))); return true; }
    catch (e) { return false; }
  }

  function unkAdd(code, note) {
    code = String(code || '').trim();
    if (!code) return false;
    var l = unkList();
    var hit = null;
    for (var i = 0; i < l.length; i++) if (String(l[i].code) === code) { hit = l[i]; break; }
    if (hit) {
      hit.n = (hit.n || 1) + 1;                 /* neçə dəfə rast gəlinib */
      hit.ts = Date.now();
      if (note) hit.note = note;
    } else {
      l.unshift({
        code: code, note: note || '', n: 1,
        ts: Date.now(), who: (role() || 'kassa'),
        done: false
      });
    }
    return unkSave(l);
  ...
  /* Kassirin gördüyü ekran — bir xana, bir düymə */
  function doReport(code) {
    paint('<div class="cc-card cc-warn">' +
      '<div class="cc-h">📮 Tanınmayan kod</div>' +
      '<div class="cc-p">Kod sistemdə yoxdur. Bura yaz — admin sonra malı yaradacaq. ' +
      'Sən işini davam etdir.</div>' +
      '<div class="cc-f"><span>Kod</span>' +
        '<input id="unkCode" inputmode="numeric" value="' + esc(code || '') + '" ' +
        'placeholder="barkod və ya kod"></div>' +
      '<div class="cc-f"><span>Qeyd</span>' +
        '<input id="unkNote" placeholder="malın adı, rəngi… (istəyə görə)"></div>' +
      '<div class="cc-row">' +
        '<button class="cc-btn cc-pri" onclick="JollyCC.reportSend()">📮 Adminə göndər</button>' +
        '<button class="cc-btn" onclick="JollyCC.cancel()">Ləğv et</button>' +
      '</div></div>');
    setTimeout(function () {
      var el = document.getElementById(code ? 'unkNote' : 'unkCode');
      if (el) { try { el.focus(); } catch (e) {} }
    }, 80);
  }

  /* Adminin gördüyü siyahı */
  function doUnknown() {
    if (isWorker()) return doReport('');
    var l = unkList();
    var open = l.filter(function (x) { return !x.done; });
    var done = l.filter(function (x) { return x.done; });

    if (!l.length) {
      paint('<div class="cc-card cc-ok"><div class="cc-h">✅ Bildiriş yoxdur</div>' +
        '<div class="cc-p">Kassirlər tanınmayan kod bildirməyib.</div></div>');
      return;
    }
    ...
    var row = function (x) {
      ...
      return '<div class="cc-frow' + (x.done ? ' done' : '') + '">' +
        '<button class="cc-fcode" draggable="true" data-v="' + esc(x.code) + '" ' +
          'onclick="JollyCC.grab(this)" ondragstart="JollyCC.drag(event,this)">⧉ ' +
          esc(x.code) + '</button>' +
        '<span class="cc-flab">' + (x.note ? esc(x.note) + ' · ' : '') +
          when + (x.n > 1 ? ' · ' + x.n + ' dəfə' : '') + '</span>' +
        (x.done ? '' :
          '<button class="cc-act cc-fnew" style="flex:none;width:44px" title="Mal yarat" ' +
            'onclick="JollyCC.newWith(\'' + esc(x.code) + '\')">＋</button>' +
          '<button class="cc-act" style="flex:none;width:40px" title="Həll olundu" ' +
            'onclick="JollyCC.unkDone(\'' + esc(x.code) + '\')">✓</button>') +
        '<button class="cc-act" style="flex:none;width:38px" title="Sil" ' +
          'onclick="JollyCC.unkDrop(\'' + esc(x.code) + '\')">✕</button>' +
      '</div>';
    };

    paint(
      '<div class="cc-note">📮 ' + open.length + ' həll gözləyir · ' +
        done.length + ' həll olunub</div>' +
      (open.length ? '<div class="cc-sec">Həll gözləyir</div>' +
        '<div class="cc-folder">' + open.map(row).join('') + '</div>' : '') +
      (done.length ? '<div class="cc-sec">Həll olunub</div>' +
        '<div class="cc-folder">' + done.map(row).join('') + '</div>' : ''));
  }
  ...
  function css() {
    ...
    st.textContent = [
      ...
      '.cc-exit{position:fixed;right:12px;bottom:12px;z-index:70;width:46px;height:46px;',
      'border-radius:50%;cursor:pointer;font-size:19px;opacity:.5;',
      'background:rgba(255,255,255,.9);border:1.5px solid rgba(0,0,0,.12);',
      'display:flex;align-items:center;justify-content:center}',
      '.cc-exit:active{opacity:1}',
  ...
  function render() {
    ...
    var unkOpen = unkList().filter(function (x) { return !x.done; }).length;
    if (isWorker()) {
      pills.unshift('<span class="cc-pill" onclick="JollyCC.report(\'\')">' +
        '📮 Tanınmayan kod bildir</span>');
    } else if (unkOpen) {
      pills.push('<span class="cc-pill" onclick="JollyCC.unknown()">📮 ' +
        unkOpen + ' bildiriş</span>');
    }
    ...
    return '<div class="storeos cc-wrap' + (isWorker() ? ' cc-worker' : '') + '">' +
      ...
      '<div class="cc-hint">' +
        (isWorker()
          ? '<b>545</b> → barkodla tap · <b>corab</b> → adla tap · ' +
            '<b>F2</b> barkodu kopyala · <b>F3</b> son mal · ' +
            '<b>çıxış</b> yaz → hesabı dəyiş'
      ...
      /* ★ Çıxış həmişə əlçatan olsun — kassir kilidində belə */
      (isWorker()
        ? '<button class="cc-exit" title="Hesabı dəyiş" onclick="JollyCC.exit()">🔑</button>'
        : '') +
      '<div class="cc-split">' +
        '<div class="cc-left">' +
  ...
  global.JollyCC = {
    ...
    tint: tintOf,
    diag: diag,
    hideStrays: hideStrays,
    report: doReport,
    reportSend: function () {
      var c = document.getElementById('unkCode');
      var n = document.getElementById('unkNote');
      var code = c ? String(c.value || '').trim() : '';
      if (!code) { toast('Kod boş ola bilməz', 'error'); return; }
      if (unkAdd(code, n ? String(n.value || '').trim() : '')) {
        toast('📮 adminə göndərildi', 'ok');
        paint('<div class="cc-card cc-ok"><div class="cc-h">✅ Göndərildi</div>' +
          '<div class="cc-p"><b>' + esc(code) + '</b> siyahıya düşdü. ' +
          'Admin baxıb malı yaradacaq.</div>' +
          '<div class="cc-row"><button class="cc-btn" onclick="JollyCC.report(\'\')">' +
          '📮 Daha birini yaz</button></div></div>');
      } else toast('Yazıla bilmədi', 'error');
    },
    unknown: doUnknown,
    unkDone: function (c) {
      var l = unkList();
      l.forEach(function (x) { if (String(x.code) === String(c)) x.done = true; });
      unkSave(l); doUnknown();
    },
    unkDrop: function (c) {
      unkSave(unkList().filter(function (x) { return String(x.code) !== String(c); }));
      doUnknown();
    },
    unkList: unkList,
    exit: doExit,
    ...
    pinOk: function () {
      ...
      if (!v) { toast('PIN yaz', 'error'); return; }
      if (hash4(v) !== ccPin()) { toast('PIN səhvdir', 'error'); return; }
      exitNow();
    },
    pinSet: function () {
      var el = document.getElementById('ccPinNew');
      var v = el ? String(el.value || '').trim() : '';
      if (v.length < 4) { toast('Ən azı 4 rəqəm', 'error'); return; }
      try { localStorage.setItem(PIN_KEY, hash4(v)); } catch (e) {}
      toast('🔒 PIN quruldu', 'ok');
      exitNow();
    },
    pinClear: function () {
      try { localStorage.removeItem(PIN_KEY); } catch (e) {}
      toast('PIN silindi', 'ok');
    },
    hasPin: function () { return !!(appPin() || ccPin()); },
    exitHard: function () {
      try { sessionStorage.removeItem('jolly_sec_session'); } catch (e) {}
  ...
  var HIDE = [
    ...
    '[class*="edge-"]', '[id*="edge"]',
    /* ★ Giriş nişanı (Kassa | ⟳) yuxarı solda ekranı örtürdü.
       Gizlətmirik — kassir kim olduğunu bilməlidir — sağ AŞAĞI
       küncə köçürürük, işin qarşısını kəsməsin. */
    '.backup-dot', '.icon-btn',
  ...
  /* ★ 2026-08-23 — Esqin: "işarə qaldı orda, onu sil".
     Selektor sayaraq təqib etmək bitmir: hər faylın öz adı var.
     ONA GÖRƏ TƏRSİNƏ: ekranda SABİT duran (position:fixed/sticky)
     hər elementi yoxlayırıq — bizim deyilsə və üst zolaq deyilsə,
     gizlədirik. Ad bilməyə ehtiyac qalmır. */
  function hideStrays() {
    ...
    try {
      ...
      for (var i = 0; i < kids.length; i++) {
        var el = kids[i];
        if (!el || el === main || el.id === SHELL_ID) continue;
        if (el.tagName === 'SCRIPT' || el.tagName === 'STYLE' ||
            el.tagName === 'LINK' || el.tagName === 'META') continue;
        if (el.id === 'ccZoom' || el.id === 'ccPhoto') continue;   /* bizimdir */
        try { if (el.className && String(el.className).indexOf('cc-exit') !== -1) continue; }
        catch (e) {}
        if (main && main.contains && main.contains(el)) continue;

        var cs = null;
        try { cs = global.getComputedStyle ? global.getComputedStyle(el) : null; } catch (e) {}
        if (!cs) continue;
  ...
  setInterval(function () {
    guardWorker(); ensureScreen(); keepFocus(); closeStray(); hideStrays();
  }, 900);
