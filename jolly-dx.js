...
(function (global) {
  ...
  register({
    ...
    test: function () {
      ...
      if (total > 4300000) return crit('yaddaş dolmaq üzrədir (~5 MB hədd)', mb + ' MB · ' + txt);
    ...
    test: function () {
      ...
      try {
        for (var i = 0; i < localStorage.length; i++) {
          ...
          if (/key|token|secret|password|passw|pin/i.test(k)) {
  ...
  /* ══════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════ */
  function snapshot() {
    var keys = [], data = {}, bytes = 0;
    try {
      for (var i = 0; i < localStorage.length; i++) {
        var k = localStorage.key(i);
        if (k.indexOf('jolly') !== 0 || k === SNAP_KEY) continue;
        var v = localStorage.getItem(k) || '';
        if (v.length > 400000) continue;
      ...
      return { ok: true, id: snap.id, keys: keys.length, kb: Math.round(bytes / 1024) };
    } catch (e) {
