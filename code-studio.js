...
const JollyCodeStudio = (() => {
  ...
  function addField() {
    const label = prompt('Yeni sahənin adı (məs. "Şüşə rəngi"):');
    ...
    const groupNames = JollyDB.Groups.all().map(g => g.name);
    const groupsRaw = prompt(`Bu sahə hansı QRUPLARDA görünsün?\n\nMövcud qruplar: ${groupNames.join(', ') || '(hələ qrup yoxdur)'}\n\nAd(lar)ı vergüllə yaz (məs. "Eynək, Günəş eynəyi"). BOŞ burax = bütün qruplarda görünsün:`);
    const groups = (groupsRaw || '').split(',').map(s => s.trim()).filter(Boolean);
    const fields = getFields();
    fields.push({ id: JollyDB.uid('fld'), key: 'cf_' + Date.now().toString(36), label: label.trim(), type, options, groups });
    setFields(fields);
    Toast.success(groups.length ? `"${label}" sahəsi əlavə olundu — yalnız ${groups.join(', ')} qrupunda görünəcək` : `"${label}" sahəsi əlavə olundu — bütün qruplarda görünəcək`);
  ...
  function editField(id) {
    ...
    const groupNames = JollyDB.Groups.all().map(g => g.name);
    const groupsRaw = prompt(`Hansı QRUPLARDA görünsün?\n\nMövcud qruplar: ${groupNames.join(', ') || '(hələ qrup yoxdur)'}\n\nBOŞ burax = bütün qruplarda görünsün:`, (f.groups || []).join(', '));
    const groups = groupsRaw === null ? (f.groups || []) : groupsRaw.split(',').map(s => s.trim()).filter(Boolean);
    const updated = fields.map(x => x.id === id ? { ...x, label: label.trim(), options, groups } : x);
    setFields(updated);
  ...
  /* Məhsul formasına əlavə HTML — products.js buradan çağırır.
     YENİ (2026-07-22): sahə yalnız MÜƏYYƏN qruplar üçün təyin
     olunubsa, məhsulun qrupu uyğun gəlmirsə göstərilmir. "groups"
     boşdursa (köhnə sahələr), bütün qruplarda görünməyə davam edir. */
  function renderCustomFieldsHtml(product) {
    const productGroup = product && product.group;
    const fields = getFields().filter(f => !f.groups || !f.groups.length || (productGroup && f.groups.includes(productGroup)));
  ...
  /* Formadan custom dəyərləri oxu — YENİ: hazırkı qrup üçün
     göstərilməyən (fərqli qrupa aid) sahələrin köhnə dəyərini
     itirmədən saxlayır, yalnız EKRANDA olanları yeniləyir. */
  function collectCustomValues(container, existing) {
    const fields = getFields();
    ...
    return `
      ...
      <div class="glass" style="padding:4px 14px;">
        ${fields.length ? fields.map(f => `
          <div class="list-row">
            <span>${escapeAttr(f.label)} <span class="muted" style="font-size:11px;">(${FIELD_TYPES[f.type]})</span><br><span class="mono muted" style="font-size:10px;">${(f.groups && f.groups.length) ? '🏷️ ' + escapeAttr(f.groups.join(', ')) : '🌐 bütün qruplar'}</span></span>
