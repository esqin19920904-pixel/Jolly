...
(function (global) {
  ...
  function ask(q) {
    ...
    return fetch(endpoint(), opt)
      ...
      .then(function (j) {
        ...
        if (!j || !j.ok) {
          /* Server `detail` göndərirsə — əsl səbəb odur, onu göstəririk */
          var msg = (j && j.error) || 'Cavab alınmadı';
          if (j && j.detail) msg += ' — ' + String(j.detail).slice(0, 180);
  ...
  /* Bağlantını yoxlamaq — Diaqnostika və Studio çağırır */
  function test() {
  ...
  /* ★ Cavab HƏMİŞƏ söhbətin içində qalmalıdır.
     Əvvəl `document.body`-yə düşürdü: cavab 2-3 saniyəyə gəlir,
     bu müddətdə söhbət yenidən çəkilir, bağlandığım baloncuq
     DOM-dan çıxır və `parentNode` boş olur. Onda kod cavabı
     səhifənin ən dibinə atırdı.
     İndi: baloncuq yerindədirsə onun ardınca, yoxdursa əvvəlcədən
     yadda saxladığım söhbət qutusunun SONUNA qoyulur. */
  function attached(el) {
    try { return !!(el && document.body && document.body.contains(el)); } catch (e) { return false; }
  }

  function place(box, afterEl, host) {
    if (attached(afterEl) && afterEl.parentNode) {
      afterEl.parentNode.insertBefore(box, afterEl.nextSibling);
      return true;
    }
    if (attached(host)) { host.appendChild(box); return true; }
    var main = document.getElementById('main');
    if (attached(main)) { main.appendChild(box); return true; }
    return false;                       /* body-yə HEÇ VAXT atmırıq */
  }

  function appendAnswer(afterEl, text, host) {
    try {
      ...
      if (!place(box, afterEl, host)) return;
      try { box.scrollIntoView({ block: 'nearest' }); } catch (e) {}
    } catch (e) {}
  }

  /* Kiçik izah sətri — cavab yerinə, uğursuzluğun səbəbi */
  function appendNote(afterEl, text, kind, host) {
    try {
      clearNote(afterEl);
      var b = document.createElement('div');
      b.setAttribute('data-aib-note', '1');
      b.style.cssText = 'margin:6px 0;padding:8px 11px;border-radius:11px;font-size:11.5px;' +
        'line-height:1.5;opacity:.8;background:' +
        (kind === 'wait' ? 'rgba(147,197,253,.1)' : 'rgba(251,191,36,.12)') +
        ';border:1px solid ' + (kind === 'wait' ? 'rgba(147,197,253,.25)' : 'rgba(251,191,36,.28)');
      b.textContent = text;
      place(b, afterEl, host);
    } catch (e) {}
  }

  function clearNote() {
    /* Qeyd köçmüş ola bilər — harada olsa tapıb silirik */
    try {
      var all = document.querySelectorAll('[data-aib-note]');
  ...
  function handleNode(el) {
    ...
    if (el.getAttribute && el.getAttribute('data-aib-note')) return;
    ...
    /* ★ SƏSSİZ QALMIRIQ. Əvvəl uğursuzluqda heç nə göstərilmirdi,
       ona görə səbəbi bilmək mümkün deyildi — körpü işləmirmi,
       sual tutulmayıbmı, yoxsa server cavab vermir. İndi hər halda
       bir sətir yazılır. */
    if (!lastQ) {
      appendNote(el, 'sual tutulmadı — ➤ düyməsi ilə və ya Enter ilə göndər', null, el.parentNode);
      return;
    }

    var q = lastQ;
    /* Söhbət qutusunu İNDİ yadda saxlayırıq — cavab gələndə
       baloncuq artıq yox ola bilər */
    var host = el.parentNode;
    busy = true;
    appendNote(el, '☁️ soruşulur…', 'wait', host);
    ask(q).then(function (r) {
      busy = false;
      clearNote();
      if (r && r.ok && r.text) appendAnswer(el, r.text, host);
      else appendNote(el, '☁️ AI cavab vermədi — ' + ((r && r.error) || 'səbəb bilinmir'), null, host);
    }).catch(function (e) {
      busy = false;
      clearNote();
      appendNote(el, '☁️ AI xətası — ' + ((e && e.message) || 'naməlum'), null, host);
    });
  }
