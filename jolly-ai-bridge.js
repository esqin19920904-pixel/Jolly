...
(function (global) {
  ...
  function appendNote(afterEl, text, kind) {
    try {
      clearNote(afterEl);
      ...
      b.textContent = text;
      if (afterEl.parentNode) afterEl.parentNode.insertBefore(b, afterEl.nextSibling);
    } catch (e) {}
  }

  function clearNote(afterEl) {
    try {
      var n = afterEl.nextSibling;
      if (n && n.getAttribute && n.getAttribute('data-aib-note')) n.parentNode.removeChild(n);
    } catch (e) {}
  }
  ...
  function handleNode(el) {
    ...
    /* ★ SƏSSİZ QALMIRIQ. Əvvəl uğursuzluqda heç nə göstərilmirdi,
       ...
       bir sətir yazılır. */
    if (!lastQ) {
      appendNote(el, 'sual tutulmadı — ➤ düyməsi ilə və ya Enter ilə göndər');
      return;
    }

    var q = lastQ;
    busy = true;
    appendNote(el, '☁️ soruşulur…', 'wait');
    ask(q).then(function (r) {
      busy = false;
      clearNote(el);
      if (r && r.ok && r.text) appendAnswer(el, r.text);
      else appendNote(el, '☁️ AI cavab vermədi — ' + ((r && r.error) || 'səbəb bilinmir'));
    }).catch(function (e) {
      busy = false;
      clearNote(el);
      appendNote(el, '☁️ AI xətası — ' + ((e && e.message) || 'naməlum'));
    });
  }
