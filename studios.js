...
const JollyStudios = (() => {
  const LIST = [
    ...
    { key: 'changelog', icon: '🆕', title: 'Yeniliklər', sub: '"Nə dəyişdi?" idarəetməsi', ready: true, route: '#/studios/changelog' },
  ];
  ...
  /* DÜZƏLİŞ (2026-07-23, yaddaş kvotası — 4-cü tur):
     navigator.storage.estimate() IndexedDB+Cache kimi MÜASİR saxlama
     sahəsini ölçür (adətən GB-larla) — köhnə localStorage-ın ÖZÜNÜN,
     brauzerdə çox vaxt 5-10MB-la sərt məhdudlaşan, AYRI kvotası var
     və bu rəqəmdə görünmür. Ona görə "10241 MB-dan 0.6 MB istifadə
     olunub" göstərsə də, əsl QuotaExceededError localStorage-ın öz
     kiçik limitindən gələ bilər. Bu, əsl localStorage ölçüsünü
     (bütün açarlar üzrə, ən böyüklərini ayrıca göstərməklə) hesablayır. */
  function loadRealLocalStorageSize() {
    const el = document.getElementById('realLsSize');
    if (!el) return;
    try {
      let total = 0;
      const rows = [];
      for (let i = 0; i < localStorage.length; i++) {
        ...
        const bytes = (k.length + v.length) * 2; // UTF-16 təxmini
        total += bytes;
        rows.push({ k, bytes });
      }
      ...
      el.innerHTML = `
        <div>🧮 Əsl localStorage ölçüsü: <b>${totalKB} KB</b> (${rows.length} açar)</div>
        <div style="margin-top:6px;">${top.map(r => `<div style="display:flex;justify-content:space-between;font-size:11px;padding:2px 0;"><span class="mono">${JollyProducts.escapeHtml(r.k)}</span><span>${(r.bytes/1024).toFixed(1)} KB</span></div>`).join('')}</div>
      `;
    } catch (e) {
      el.textContent = 'Hesablana bilmədi: ' + (e && e.message || e);
    }
  }
  ...
  function loadStorageEstimate() {
    ...
    if (navigator.storage && navigator.storage.estimate) {
      navigator.storage.estimate().then(est => {
        ...
        el.innerHTML = `💾 Ümumi sayt yaddaşı (IndexedDB+Cache): <b>${usedMB} MB</b> / ${quotaMB} MB (${pct}%)`;
  ...
  const EDGE_CATALOG = [
    ...
    { id: 'barcodeFolder', label: 'Barkod Qovluğu', icon: '📁', route: '#/barcode-folder' },
  ];
  ...
  const TOMBSTONE_TYPE_LABELS = {
    jolly_groups: 'Qrup', jolly_brands: 'Firma', jolly_locations: 'Yer',
    jolly_statuses: 'Status', jolly_suppliers: 'Tədarükçü', jolly_changelog: 'Yenilik',
  };

  function renderRecentlyDeleted() {
    if (typeof JollyDB.getTombstones !== 'function') return '<div class="muted" style="padding:14px;">Modul yüklənməyib</div>';
    const list = JollyDB.getTombstones();
    if (!list.length) return '<div class="muted" style="padding:14px;">Son 7 gündə heç nə silinməyib.</div>';
    return list.map(t => {
      const typeLabel = TOMBSTONE_TYPE_LABELS[t.key] || t.key;
      const when = new Date(t.deletedAt).toLocaleString('az-AZ');
      return `
        <div class="list-row">
          <span>🗑️ ${JollyProducts.escapeHtml(t.name || '(adsız)')} <span class="muted" style="font-size:10.5px;">— ${typeLabel} · ${when}</span></span>
          ${t.snapshot ? `<span onclick="JollyStudios.restoreDeletedItem('${t.key}','${t.id}')" style="cursor:pointer;color:var(--accent-2);font-size:12px;">♻️ Bərpa et</span>` : `<span class="muted" style="font-size:10.5px;">bərpa mümkün deyil</span>`}
        </div>
      `;
    }).join('');
  }

  function restoreDeletedItem(key, id) {
    if (!confirm('Bu qeyd geri qaytarılsın?')) return;
    const ok = JollyDB.restoreTombstone(key, id);
    if (ok) Toast.success('Bərpa olundu ♻️'); else Toast.error('Bərpa alınmadı');
    const zone = document.getElementById('recentlyDeletedZone');
    if (zone) zone.innerHTML = renderRecentlyDeleted();
  }
  ...
  function renderData() {
    ...
    setTimeout(() => loadRealLocalStorageSize(), 0);
    ...
    return `
      ...
      <div class="section-title">🗑️ Son silinənlər (7 gün)</div>
      <div class="glass" style="padding:4px 14px;margin-bottom:14px;" id="recentlyDeletedZone">
      ...
      <div class="glass" style="padding:14px;">
        ...
        <div id="realLsSize" class="muted" style="font-size:11.5px;margin-top:10px;padding-top:10px;border-top:1px solid var(--border-soft);">Hesablanır...</div>
      </div>
  ...
  /* DÜZƏLİŞ (2026-07-23, yaddaş kvotası problemi):
     Əvvəl JollyDB.exportAll() ilə HƏR ŞEYİ (məhsullar + tombstone-lar +
     fəaliyyət jurnalı + icazə jurnalı + dəyişiklik jurnalı və s.) gündə
     bir dəfə İKİNCİ DƏFƏ localStorage-a ('jolly_snapshot' açarı altında)
     yazırdıq. Tombstone-lar indi hər silinən qeydin tam surətini
     daşıdığı üçün bu, demək olar ki, bütün datanın ikiqat yer tutmasına
     səbəb olub və brauzerin yaddaş kvotasını doldurub ("Yaddaş dolub —
     məlumat saxlanmadı!" xəbərdarlığı).
     İndi snapshot YALNIZ əsl kataloq datasını saxlayır (məhsul, firma,
     qrup, yer, status, tədarükçü, etiket, ayarlar, qaralama, səbət) —
     tombstone/fəaliyyət/icazə-jurnalı/dəyişiklik-jurnalı kimi ağır və
     onsuz da öz yerlərində saxlanan massivlər xaric edilir. Məqsəd eyni
     qalır: gündəlik qəza-bərpa nöqtəsi — bunun üçün bu sahələr kifayətdir. */
  function saveSnapshot() {
    try {
      const K = JollyDB.KEYS;
      const data = {
        [K.products]: JollyDB.read(K.products, []),
        [K.brands]: JollyDB.read(K.brands, []),
        [K.groups]: JollyDB.read(K.groups, []),
        [K.locations]: JollyDB.read(K.locations, []),
        [K.statuses]: JollyDB.read(K.statuses, []),
        [K.suppliers]: JollyDB.read(K.suppliers, []),
        [K.tags]: JollyDB.read(K.tags, []),
        [K.settings]: JollyDB.read(K.settings, {}),
        [K.drafts]: JollyDB.read(K.drafts, []),
        [K.trash]: JollyDB.read(K.trash, []),
