...
const JollyAdminStudio = (() => {
  ...
  function renderRow(key, it) {
    ...
    return `
      <div class="list-row" draggable="true" data-id="${it.id}">
        <span ${key === 'groups' ? `onclick="JollyRouter.go('#/products?group=${encodeURIComponent(it.name)}')" style="cursor:pointer;"` : ''}>${groupIcon}${colorDot}${codePrefix}${JollyProducts.escapeHtml(it.name)}${minImagesBadge}${barcodePriceBadge}</span>
        <span class="actions">
  ...
  return { renderHome, renderList, addItem, editItem, deleteItem, editIcon, editMinImages, editBarcodePrice, quickCapture, _captureShot, stopQuickCapture, STORE_MAP };
})();
