...
/* ── SQL sxemi ─────────────────────────────────────────────────
   ...
   ─────────────────────────────────────────────────────────── */
/* ★★ 2026-08-20 — CƏDVƏL ADLARI DƏYİŞDİRİLDİ
   ────────────────────────────────────────────────────────────
   SƏHV: JOLLY `products` adlı cədvələ yazırdı. Seçilmiş D1
   bazasında həmin adda cədvəl ONSUZ DA var idi — Kodsuz
   Mehsullar-ınkı. Nəticədə iki proqramın malları QARIŞDI.

   Xəbərdarlıq siqnalı var idi: "no such column: deleted".
   Mən sütun əlavə etməklə keçdim, əvəzinə cədvəlin kimə aid
   olduğunu soruşmalı idim.

   İNDİ: bütün cədvəllər `jolly_` ilə başlayır. İki proqram
   eyni bazada yan-yana yaşayır, bir-birinə toxunmur.
   Kodsuz Mehsullar-ın məlumatına HEÇ NƏ olmur. */
const SCHEMA = [
  `CREATE TABLE IF NOT EXISTS jolly_products (
     id          TEXT PRIMARY KEY,
  ...
  `CREATE INDEX IF NOT EXISTS ix_jolly_products_updated ON jolly_products(updated_at)`,
  `CREATE INDEX IF NOT EXISTS ix_jolly_products_name    ON jolly_products(name)`,

  /* Barkod üzrə sürətli axtarış üçün ayrıca cədvəl.
     Bir malın bir neçə barkodu ola bilər. */
  `CREATE TABLE IF NOT EXISTS jolly_barcodes (
     code       TEXT NOT NULL,
     product_id TEXT NOT NULL,
     PRIMARY KEY (code, product_id)
   )`,
  `CREATE INDEX IF NOT EXISTS ix_jolly_barcodes_pid ON jolly_barcodes(product_id)`,

  `CREATE TABLE IF NOT EXISTS jolly_users (
     id         TEXT PRIMARY KEY,
     name       TEXT NOT NULL,
  ...
  `CREATE TABLE IF NOT EXISTS jolly_meta (
     k TEXT PRIMARY KEY,
     v TEXT
   )`,
];

/* ── Sütunların gözlənilən siyahısı ─────────────────────────
   `CREATE TABLE IF NOT EXISTS` MÖVCUD cədvələ toxunmur. Əgər
   bazada həmin adda köhnə cədvəl varsa (başqa layihədən qalmış),
   bizim yeni sütunlar olmur və sorğu "no such column" verir.
   Esqinin bazasında məhz belə oldu: `deleted` sütunu yox idi.

   Ona görə hər açılışda mövcud sütunlar yoxlanılır və
   çatışmayanlar ALTER TABLE ilə əlavə olunur. Məlumat itmir. */
const COLUMNS = {
  jolly_products: {
    name: "TEXT NOT NULL DEFAULT ''",
    price: 'REAL',
    brand: 'TEXT', grp: 'TEXT', supplier: 'TEXT', location: 'TEXT',
  ...
  jolly_barcodes: { code: 'TEXT', product_id: 'TEXT' },
};
...
async function migrate(env) {
  ...
  for (const table of Object.keys(COLUMNS)) {
    ...
    try {
      ...
      have = (info.results || []).map(r => r.name);
    } catch (e) { continue; }
    if (!have.length) continue;               /* cədvəl yoxdursa SCHEMA yaradır */

    for (const [col, type] of Object.entries(COLUMNS[table])) {
      if (have.includes(col)) continue;
      try {
        await env.DB.prepare(`ALTER TABLE ${table} ADD COLUMN ${col} ${type}`).run();
        added.push(table + '.' + col);
      } catch (e) { /* paralel sorğu artıq əlavə etmiş ola bilər */ }
    }
  }
  return added;
}
...
async function upsertProduct(env, p) {
  ...
  await env.DB.prepare(`DELETE FROM jolly_barcodes WHERE product_id = ?`).bind(id).run();
  for (const code of bc) {
    await env.DB.prepare(
      `INSERT OR IGNORE INTO jolly_barcodes (code, product_id) VALUES (?,?)`
    ).bind(code, id).run();
  }
  return { ok: true, id };
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      /* ── Vəziyyət ─────────────────────────────────────────── */
      if (path === '/api/ping') {
        let count = null, fixed = [];
        try {
          if (env.DB) {
            schemaReady = false;              /* ping həmişə yoxlasın */
            fixed = (await ensureSchema(env)) || [];
            schemaReady = true;
            const r = await env.DB.prepare(
              `SELECT COUNT(*) AS n FROM jolly_products WHERE deleted = 0`).first();
            count = r ? r.n : null;
          }
        } catch (e) { count = 'xəta: ' + String(e && e.message).slice(0, 80); }

        return json({
          ok: true, app: 'JOLLY', wv: '2026-08-20-own-tables',
          db: !!env.DB, media: !!env.MEDIA, assets: !!env.ASSETS,
          products: count, fixed: fixed.length ? fixed : undefined,
          at: Date.now(),
        });
      }
      ...
      if (path === '/api/products' && M === 'GET') {
        ...
        const rows = await env.DB.prepare(
          `SELECT * FROM jolly_products WHERE updated_at > ? ORDER BY updated_at ASC LIMIT ?`
      ...
      if (path === '/api/bc' && M === 'GET') {
        ...
        const rows = await env.DB.prepare(
          `SELECT p.* FROM jolly_products p
             JOIN jolly_barcodes b ON b.product_id = p.id
      ...
      if (path === '/api/users' && M === 'GET') {
        ...
        await withSchema(env);
        const rows = await env.DB.prepare(
          `SELECT * FROM jolly_users WHERE deleted = 0 ORDER BY name`).all();
        return json({
          ok: true,
      ...
      if (path === '/api/users/push' && M === 'POST') {
        ...
        for (const u of list) {
          ...
          await env.DB.prepare(
            `INSERT INTO jolly_users (id,name,role,pin,extra,updated_at,deleted)
             VALUES (?,?,?,?,?,?,?)
             ON CONFLICT(id) DO UPDATE SET
      ...
      if (path === '/api/stats' && M === 'GET') {
        ...
        const r = await env.DB.prepare(
          `SELECT
           ...
           FROM jolly_products WHERE deleted = 0`
        ).first();
        return json({ ok: true, stats: r || {} });
      }

      /* ══════════════════════════════════════════════════════
         🧹 QARIŞIQLIĞI TƏMİZLƏMƏK
         ──────────────────────────────────────────────────────
         JOLLY səhvən Kodsuz Mehsullar-ın `products` cədvəlinə
         də yazmışdı. Bu marşrut həmin sətirləri tapır və silir.

         ★ TƏHLÜKƏSİZLİK: yalnız İD-si `jolly_products`-da DA
         olan sətirlər silinir — yəni yalnız JOLLY-nin yazdıqları.
         Kodsuz-un öz mallarına toxunulmur.

         Əvvəl `?dry=1` ilə SİYAHIYA bax, sonra `?confirm=SIL`
         ilə sil.
         ══════════════════════════════════════════════════════ */
      if (path === '/api/cleanup/legacy') {
        if (!env.DB) return bad('D1 yoxdur', 503);
        await withSchema(env);
        ...
        try {
          const r = await env.DB.prepare(
            `SELECT p.id, p.name FROM products p
               JOIN jolly_products j ON j.id = p.id
              LIMIT 2000`).all();
          old = r.results || [];
        } catch (e) {
          return json({ ok: true, note: 'köhnə `products` cədvəli yoxdur — təmizləməyə ehtiyac yoxdur' });
        }

        if (url.searchParams.get('confirm') !== 'SIL') {
          return json({
            ok: true, dry: true, willDelete: old.length,
            sample: old.slice(0, 20),
            note: 'Silmək üçün ünvanın sonuna ?confirm=SIL yaz',
          });
        }
