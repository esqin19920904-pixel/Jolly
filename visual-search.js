...
const JollyVisualSearch = (() => {
  ...
  const CACHE_DB = 'jolly_vs', FP = 'fp3', LEARN = 'learn', HIST = 'hist';
  ...
  let _stats = null, _cache = null, _dirty = [], _learn = null, _lastQ = null, _lastResults = null;
  ...
  function openDB() {
    return new Promise((res, rej) => {
      ...
      const r = indexedDB.open(CACHE_DB, 4);
      r.onupgradeneeded = () => {
        const db = r.result;
  ...
  async function scan(dataUrl, opts) {
    ...
    /* 7 + 9 — OCR AXTARIŞI GÖZLƏTMİR (v3.1).
       Qısa müddət (standart 1.5 san) gözlənilir: hazır olsa bal dərhal
       qatılır. Gecikirsə vizual nəticə göstərilir, OCR fonda davam edir
       və bitəndə `ocrPending` vasitəsilə çağıran tərəf applyText() ilə
       sıranı yeniləyir. Beləliklə Tesseract-ın yüklənməsi (2 MB) heç
       vaxt istifadəçini gözlətmir. */
    ...
    if (opts.ocr !== false) {
      ...
      const wait = (typeof opts.ocrWait === 'number') ? opts.ocrWait : 1500;
      const early = await Promise.race([job, new Promise(r => setTimeout(() => r('__late__'), wait))]);
      if (early && early !== '__late__') {
        tokens = early.tokens; brand = early.brand; text = early.text;
      } else if (early === '__late__') {
        stats.ocr = 'gecikdi — fonda davam edir';
        ocrPending = job;
      }
    }
    ...
    _lastResults = final;
    return { results: final, stats: stats, barcode: barcode, text: text,
             brand: brand, viaBarcode: false, ocrPending: ocrPending };
  }

  /* OCR gec gəlirsə — bütün siyahını yenidən ballayır (yalnız ilk 5-i deyil) */
  function applyText(tokens, brand, limit) {
    const src = _lastResults || [];
    const out = src.map(r => {
      const kind = productHasText(r.product, tokens || [], brand);
      if (!kind) return r;
      const has = (r.bonuses || []).some(b => b.indexOf('marka') === 0 || b.indexOf('yazı') === 0);
      if (has) return r;
      const add = (kind === 'brand') ? B_BRAND : Math.round(B_BRAND / 2);
      const bonuses = (r.bonuses || []).slice();
      bonuses.push((kind === 'brand' ? 'marka +' : 'yazı +') + add);
      return Object.assign({}, r, {
        similarity: Math.max(0, Math.min(100, r.similarity + add)),
        bonuses: bonuses
      });
    });
  ...
  async function findBest(dataUrl, opts) {
    ...
    return {
      ...
      brand: out.brand, viaBarcode: out.viaBarcode, ocrPending: out.ocrPending
    };
  }
  ...
  const history = {
    add: function (entry) {
      const rec = {
        ...
        similarity: entry.similarity || 0, via: entry.via || 'vizual',
  ...
  /* nə qədər şəklin barmaq izi hazır deyil */
  async function warmupNeeded() {
    const cache = await loadCache();
    let total = 0, missing = 0;
    products().forEach(p => {
      imagesOf(p).forEach(raw => { total++; if (!cache.has(fnv(raw))) missing++; });
    });
  ...
  return {
    ...
    warmup, warmupNeeded, confirm: confirmMatch, applyText, history,
