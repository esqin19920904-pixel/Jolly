...
const JollyVisualSearch = (() => {
  ...
  /* HƏDDLƏR — sınaq mühitindəki ölçüyə görə kalibrlənib (07-30):
       mükəmməl uyğunluq  = 100 + rəng 5 + forma 5 + variant 8 = 118
       real WhatsApp şəkli ≈ vizual 60-80 → cəm 78-98 (OCR olmadan)
     Köhnə hədd 105 idi — real şəkil ona demək olar heç vaxt çatmırdı,
     yəni "🟢 dəqiq" və avtomatik barkod açılışı işə düşmürdü. */
  const CONF_GREEN = 95, CONF_YELLOW = 72;
  ...
  /* ================== ÖZ-ÖZÜNÜ ÖLÇMƏ ==================
     Kataloqdan mal seçir, onun öz şəklini WhatsApp kimi korlayır
     (kiçildir, sıxır, kənarını kəsir) və axtarışa verir — sonra
     həmin malın hansı yerdə çıxdığını sayır.
     DÜRÜST QEYD: bu OPTİMİST ölçüdür. Sorğu şəkli kataloqdakı
     şəklin korlanmış variantıdır; başqa bucaqdan çəkilmiş əsl
     müştəri şəkli daha çətindir.
     ==================================================== */

  async function degrade(src) {
    const img = await loadImage(src);
    const w = img.naturalWidth || img.width, h = img.naturalHeight || img.height;
    const cv = document.createElement('canvas');
    const tw = Math.max(120, Math.round(w * 0.42)), th = Math.max(120, Math.round(h * 0.42));
    cv.width = tw; cv.height = th;
    const ctx = cv.getContext('2d');
    ctx.fillStyle = '#fff'; ctx.fillRect(0, 0, tw, th);
    ctx.drawImage(img, w * 0.06, h * 0.06, w * 0.88, h * 0.88, 0, 0, tw, th);
  ...
  async function benchmark(opts) {
    opts = opts || {};
    const n = opts.n || 8;
    const list = products().filter(p => imagesOf(p).length);
    if (!list.length) return { ok: false, why: 'şəkli olan mal yoxdur', tested: 0, rows: [] };

    const picked = [];
    const step = Math.max(1, Math.floor(list.length / n));
    for (let i = 0; i < list.length && picked.length < n; i += step) picked.push(list[i]);

    const rows = [];
    for (const p of picked) {
      let src = null;
      try { src = await resolveSrc(imagesOf(p)[0]); } catch (e) { src = null; }
      if (!src) { rows.push({ name: p.name || '(adsız)', rank: 0, note: 'şəkil açılmadı' }); continue; }
      let q = null;
      try { q = await degrade(src); } catch (e) { rows.push({ name: p.name || '(adsız)', rank: 0, note: 'korlanmadı' }); continue; }

      const t0 = Date.now();
      let out = null;
      try { out = await scan(q, { ocr: false, barcode: false }); }
      catch (e) { rows.push({ name: p.name || '(adsız)', rank: 0, note: 'axtarış xətası' }); continue; }

      let rank = -1;
      for (let i = 0; i < out.results.length; i++) {
        if (String(out.results[i].product.id) === String(p.id)) { rank = i + 1; break; }
      }
      rows.push({
        name: p.name || '(adsız)',
        rank: rank,
        confidence: rank > 0 ? out.results[rank - 1].confidence : 0,
        similarity: rank > 0 ? out.results[rank - 1].similarity : 0,
        topName: out.results[0] ? (out.results[0].product.name || '(adsız)') : '—',
        topConfidence: out.results[0] ? out.results[0].confidence : 0,
        ms: Date.now() - t0
      });
      if (typeof opts.onProgress === 'function') { try { opts.onProgress(rows.length, picked.length); } catch (e) {} }
      await new Promise(r => setTimeout(r, 0));
    }

    const valid = rows.filter(r => r.rank && r.rank > 0);
    const first = valid.filter(r => r.rank === 1).length;
    const top5 = valid.filter(r => r.rank <= 5).length;
    const avg = arr => arr.length ? Math.round(arr.reduce((a, b) => a + b, 0) / arr.length) : 0;

    return {
      ok: true, tested: rows.length, usable: valid.length,
      first: first, top5: top5, missed: rows.length - top5,
      firstRate: rows.length ? Math.round(first / rows.length * 100) : 0,
      avgMs: avg(rows.filter(r => r.ms).map(r => r.ms)),
      avgConfidenceCorrect: avg(valid.filter(r => r.rank === 1).map(r => r.confidence)),
      avgConfidenceWrongTop: avg(rows.filter(r => r.rank && r.rank > 1).map(r => r.topConfidence)),
      rows: rows,
      note: 'Optimist ölçü: sorğu şəkli kataloq şəklinin korlanmış variantıdır'
    };
  }
  ...
  async function selfTest() {
    ...
    /* ⚠️ səbəbini göstərən yoxlama: bir mal alınmasa növbətini sına
       və NİYƏ alınmadığını qeyd et (şəkil IDB-dən gəlmir, format
       pozuqdur və s.) — əvvəl sadəcə "⚠️" görünürdü, səbəb yox idi */
    out.tried = 0; out.failed = [];
    const withImg = list.filter(p => imagesOf(p).length);
    for (const p of withImg.slice(0, 5)) {
      out.tried++;
      const raw = imagesOf(p)[0];
      let src = null;
      try { src = await resolveSrc(raw); }
      catch (e) { out.failed.push((p.name || p.id) + ': IDB xətası'); continue; }
      if (!src) { out.failed.push((p.name || p.id) + ': şəkil IDB-dən gəlmədi'); continue; }
      try {
        const im = await loadImage(src);
        const d = profileOf(im);
        out.sample = visualScore(d, d).score;
        out.autoCrop = d.box;
        out.sampleOf = p.name || p.id;
        break;
      } catch (e) {
        out.failed.push((p.name || p.id) + ': şəkil açılmadı (' + String(src).slice(0, 12) + '…)');
      }
    }
    if (out.sample === null) {
      out.reason = out.failed.length
        ? ('heç bir şəkil oxunmadı — ' + out.failed[0])
        : (out.withImages ? 'naməlum səbəb' : 'kataloqda şəkli olan mal yoxdur');
    }
