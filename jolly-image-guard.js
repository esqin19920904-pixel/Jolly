...
(function (global) {
  ...
  /* Fayl iki dəfə yüklənsə heç nə etməsin — index.html-də də,
     tənbəl yükləyicidə də ola bilər. */
  if (global.JollyImageGuard) return;
  ...
  function sweep(force) {
    ...
    var res = { rescued: 0, deleted: 0, waiting: 0, kept: 0 };
    ...
    for (var i = 0; i < refs.length; i++) {
      ...
      /* ★ ESQİNİN QAYDASI (08-11): "mən backup etmədən şəkillər
         silinməsin". Buludda nüsxəsi (fbs:) YOXDURSA fiziki silmə
         HEÇ VAXT baş vermir — nə qədər köhnə olsa da gözləyir.
         Yer az qalsa Şəkil Qoruyucusu ekranından əl ilə silinə bilər. */
      var hasCloud = String(ref).indexOf('fbs:') !== -1;
      if (!hasCloud && !force) { res.kept = (res.kept || 0) + 1; continue; }

      if (force || age > GRACE_MS) {    // nüsxəsi var, sahibsizdir, vaxtı keçib
        (function (r) {
  ...
  /* ══════════════════════════════════════════════════════════
     SƏBƏT (jolly_trash) ANALİZİ
     ──────────────────────────────────────────────────────────
     db.js `Trash.moveToTrash()` silinən məhsulun TAM surətini
     saxlayır — `images` sahəsi ilə birlikdə. Şəkil `data:` kimi
     içəridə saxlanılıbsa (köhnə məhsullar belədir), bir neçə
     silinmiş məhsul localStorage-ı doldura bilir.
     Esqinin cihazında: kataloqda 1 məhsul, `jolly_trash` 948 KB.
     ══════════════════════════════════════════════════════════ */
  ...
  function trashStats() {
    var raw = '';
    try { raw = localStorage.getItem('jolly_trash') || ''; } catch (e) {}
    var out = { total: bytesOf(raw), count: 0, inline: 0, inlineBytes: 0, items: [] };
    if (!raw) return out;
    var list = [];
    ...
    for (var i = 0; i < list.length; i++) {
      var it = list[i] || {};
      var sz = bytesOf(JSON.stringify(it));
      var inl = 0, inlB = 0;
      var imgs = it.images || [];
      for (var j = 0; j < imgs.length; j++) {
        if (typeof imgs[j] === 'string' && imgs[j].indexOf('data:') === 0) {
          inl++; inlB += bytesOf(imgs[j]);
        }
      }
      out.inline += inl; out.inlineBytes += inlB;
      out.items.push({ name: it.name || it.id || '?', bytes: sz, inline: inl, at: it.deletedAt || 0 });
    }
    out.items.sort(function (a, b) { return b.bytes - a.bytes; });
    return out;
  }

  /* Yalnız içəridə saxlanılan (data:) şəkilləri çıxarır.
     Məhsul qeydi səbətdə QALIR — adı, barkodu, qiyməti hamısı yerində.
     Bərpa etsən şəkilsiz bərpa olunur. */
  function stripTrashImages() {
    var list = [];
    try { list = JSON.parse(localStorage.getItem('jolly_trash') || '[]') || []; } catch (e) { return 0; }
    var removed = 0;
    for (var i = 0; i < list.length; i++) {
      var it = list[i];
      if (!it || !it.images || !it.images.length) continue;
      ...
      for (var j = 0; j < it.images.length; j++) {
        var r = it.images[j];
        if (typeof r === 'string' && r.indexOf('data:') === 0) { removed++; continue; }
        keep.push(r);
      }
      it.images = keep;
    }
    try { localStorage.setItem('jolly_trash', JSON.stringify(list)); } catch (e) { return 0; }
    return removed;
  }

  function purgeTrashOlder(days) {
    var list = [];
    try { list = JSON.parse(localStorage.getItem('jolly_trash') || '[]') || []; } catch (e) { return 0; }
    var cutoff = Date.now() - days * 864e5;
    var kept = list.filter(function (x) { return (x && x.deletedAt || 0) > cutoff; });
    var n = list.length - kept.length;
    try { localStorage.setItem('jolly_trash', JSON.stringify(kept)); } catch (e) { return 0; }
    return n;
  }
  ...
  function renderTrashPanel() {
    var t = trashStats();
    var h = [];
    h.push('<div class="section-title" style="margin-top:18px;">🗑 Səbət (silinmiş məhsullar)</div>');

    if (!t.count) {
      h.push('<div class="glass" style="padding:14px;font-size:13px;opacity:.7;">Səbət boşdur.</div>');
      return h.join('');
    }

    h.push('<div class="glass" style="padding:14px;margin-bottom:10px;">' +
             '<div style="display:flex;justify-content:space-between;padding:5px 0;font-size:12.5px;"><span>Silinmiş məhsul</span><b>' + t.count + '</b></div>' +
             '<div style="display:flex;justify-content:space-between;padding:5px 0;font-size:12.5px;"><span>Tutduğu yer</span><b>' + fmtKb(t.total) + '</b></div>' +
             '<div style="display:flex;justify-content:space-between;padding:5px 0;font-size:12.5px;"><span>İçəridə saxlanılan şəkil</span><b>' + t.inline + '</b></div>' +
             '<div style="display:flex;justify-content:space-between;padding:5px 0;font-size:12.5px;border-top:1px solid rgba(255,255,255,.08);margin-top:4px;">' +
               '<span>Şəkillərin payı</span><b style="color:' + (t.inlineBytes > 200000 ? '#fbbf24' : 'inherit') + ';">' + fmtKb(t.inlineBytes) + '</b></div>' +
           '</div>');

    if (t.inlineBytes > 50000) {
      h.push('<div class="glass" style="padding:12px 14px;margin-bottom:10px;font-size:12.5px;line-height:1.6;">' +
               'Yerin böyük hissəsini <b>silinmiş məhsulların şəkilləri</b> yeyir. Onları çıxarsan, ' +
               'məhsul qeydləri səbətdə qalır (ad, barkod, qiymət) — yalnız şəkillər gedir.' +
             '</div>');
    }

    h.push('<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px;">' +
             '<button class="btn btn-primary" onclick="JollyImageGuard.stripTrash()">🧹 Səbətdəki şəkilləri çıxar</button>' +
             '<button class="btn" onclick="JollyImageGuard.purgeTrash(7)">7 gündən köhnəni sil</button>' +
             '<button class="btn" onclick="JollyImageGuard.purgeTrash(0)">Səbəti tam boşalt</button>' +
           '</div>');

    h.push('<div class="glass" style="padding:6px 12px;margin-bottom:10px;">');
    for (var i = 0; i < Math.min(t.items.length, 15); i++) {
      var it = t.items[i];
      h.push('<div style="display:flex;gap:8px;align-items:center;padding:8px 0;font-size:12px;' +
               'border-bottom:1px solid rgba(255,255,255,.05);">' +
               '<span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + esc(it.name) + '</span>' +
               (it.inline ? '<span style="opacity:.6;">🖼' + it.inline + '</span>' : '') +
               '<span style="opacity:.55;">' + fmtKb(it.bytes) + '</span>' +
             '</div>');
    }
    if (t.items.length > 15) h.push('<div style="padding:8px 0;font-size:11.5px;opacity:.5;">… və daha ' + (t.items.length - 15) + '</div>');
    h.push('</div>');
    return h.join('');
  }
  ...
  function render() {
    ...
    h.push('<div class="dash-head"><div>' +
             ...
             '<div class="muted" style="font-size:12.5px;">v1.1 · Şəkil qorunur + səbət yaddaşı</div>' +
           '</div></div>');
  ...
  global.JollyImageGuard = {
    ...
    stripTrash: function () {
      var before = trashStats();
      var n = stripTrashImages();
      var after = trashStats();
      toast('🧹 ' + n + ' şəkil çıxarıldı · ' + fmtKb(before.total - after.total) + ' boşaldı', 'ok');
      refresh();
    },
    purgeTrash: function (days) {
      var before = trashStats();
      var n = purgeTrashOlder(days);
      var after = trashStats();
      toast('🗑 ' + n + ' qeyd silindi · ' + fmtKb(before.total - after.total) + ' boşaldı', 'ok');
      refresh();
    },
