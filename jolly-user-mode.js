...
(function (global) {
  ...
  function renderAdmin() {
    ...
    h.push('<div class="dash-head"><div>' +
             ...
             '<div class="muted" style="font-size:12.5px;">v3.3 · Karta basıb saxla — sil, gizlət, icazə ver</div>' +
           '</div></div>');
  ...
  function boot() {
    ...
    if (coreOk) {
      syncModulePerms();
      console.log('[UserMode v3.3] hazırdır — kataloq:', catalog().length, 'kart');
    ...
    if (tries > 40) {
      ...
      if (miss.length) toast('👥 v3.3 — qurulmadı: ' + miss.join(', '), 'error');
      guardRoute();
      startDomFix();
      return;
    }
