...
(function (global) {
  ...
  function renderAdmin() {
    ...
    h.push('<div class="dash-head"><div>' +
             ...
             '<div class="muted" style="font-size:12.5px;">v2.7 · Karta basıb saxla — sil, gizlət, icazə ver</div>' +
           '</div></div>');
  ...
  function boot() {
    ...
    if (coreOk && ok2) {
      syncModulePerms();
      console.log('[UserMode v2.7] hazırdır — kataloq:', catalog().length, 'kart');
    ...
    if (tries > 40) {
      ...
      if (miss.length) toast('👥 v2.7 — qurulmadı: ' + miss.join(', '), 'error');
      guardRoute();
      startDomFix();
      return;
    }
