...
(function (global) {
  ...
  function renderAdmin() {
    ...
    h.push('<div class="dash-head"><div>' +
             ...
             '<div class="muted" style="font-size:12.5px;">v2.4 · Karta basıb saxla — sil, gizlət, icazə ver</div>' +
           '</div></div>');
  ...
  function boot() {
    ...
    if (coreOk && ok2) {
      syncModulePerms();
      console.log('[UserMode v2.4] hazırdır — kataloq:', catalog().length, 'kart');
      toast('👥 İşçi Rejimi v2.4 aktivdir');
      guardRoute();
      schedulePermSync();
      startDomFix();
      return;
    ...
    if (tries > 40) {
      ...
      toast('👥 İşçi Rejimi v2.4 (qismən) aktivdir');
      guardRoute();
      startDomFix();
      return;
    }
  ...
  function startDomFix() {
    if (domT) return;
    domFix();
    domT = setInterval(domFix, 700);
    global.addEventListener('hashchange', function () { setTimeout(domFix, 120); });
  }
