...
(function (global) {
  ...
  /* ── Köhnə ayarın köçürülməsi ─────────────────────────────
     08-05-dən əvvəl bu faylın standartı "işçiyə 5 kart" idi və
     ağ siyahı yox idi. Telefonda həmin köhnə ayar qalıbsa, yeni
     standart TƏTBİQ OLUNMUR — Esqin "heç nə dəyişmədi" görürdü
     və əl ilə "Hamısını sil" basmalı olurdu.
     İndi bir dəfə avtomatik köçürülür: ağ siyahı və icazədən
     qurulma açılır, köhnə sabit kart siyahısı təmizlənir.
     Esqinin sonradan əl ilə verdiyi kartlara TOXUNULMUR —
     onlar `v` nişanı qoyulandan sonra yazılır.
     ───────────────────────────────────────────────────────── */
  var CFG_V = 3;
  var OLD_CARDS = ['home', 'scan', 'share-inbox', 'fixmode', 'tasks'].join(',');

  function migrateCfg(c) {
    if (!c || c.v === CFG_V) return c;
    c.whitelist = true;
    c.permDriven = true;
    c.simpleDash = true;
    /* Yalnız KÖHNƏ STANDART siyahını təmizləyirik */
    if (Array.isArray(c.cards) && c.cards.join(',') === OLD_CARDS) c.cards = [];
    c.v = CFG_V;
    try { localStorage.setItem(CFG_KEY, JSON.stringify(c)); } catch (e) {}
    console.log('[UserMode] köhnə ayar yeni qaydaya köçürüldü');
    return c;
  }
  ...
  /* ══════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════ */
  /* 📂 Modul Qovluğunda seçilmiş işçi — uzun basma menyusu da
     ona yazır ki, iki yerdə fərqli nəticə alınmasın.
     '*' və ya boş → ümumi (hamıya) icazə. */
  function grantUserId() {
    try {
      var id = localStorage.getItem('jolly_grant_user');
      return (id && id !== '*') ? id : null;
    } catch (e) { return null; }
  }
  function grantUserName() {
    var id = grantUserId();
    if (!id) return 'Hamısı (ümumi)';
    var U = global.JollyUsers || peek('JollyUsers');
    try {
      var all = (U && U.list) ? (U.list() || []) : [];
      for (var i = 0; i < all.length; i++) if (all[i].id === id) return all[i].name || id;
    } catch (e) {}
    return id;
  }
  ...
  function permState(key) {
    ...
    var uid = grantUserId();
    try {
      if (uid && POS.engine && typeof POS.engine.resolveFor === 'function') {
        return !!POS.engine.resolveFor(uid, key);
      }
  ...
  function setPerm(key, val) {
    if (!key) return false;
    var POS = global.POS || peek('POS');
    if (!POS || !POS.store) return false;
    var uid = grantUserId();
    try {
      if (uid && typeof POS.store.setUserOverrides === 'function') {
        var o = {}; o[key] = !!val;
        POS.store.setUserOverrides(uid, o);      // YALNIZ həmin işçiyə
      } else if (typeof POS.store.setOverride === 'function') {
        POS.store.setOverride(key, !!val);       // hamıya
      } else { return false; }
  ...
  /* ══════════════════════════════════════════════════════════
     5c) YAZI QARMAĞI — flicker-in tam həlli
     ──────────────────────────────────────────────────────────
     app.js:216 bütün səhifəni TƏK sətirlə yazır:
           mainEl().innerHTML = html;
     MutationObserver bu yazıdan SONRA işləyir — yəni səhv başlıq
     bir kadr ekrana düşür, sonra dəyişir ("gah gəlir, gah gedir").
     Ona görə #main elementinin `innerHTML` yazıcısını tuturuq:
     mətn DOM-a düşməzdən ƏVVƏL düzəldilir. Səhv başlıq heç vaxt
     görünmür. app.js-ə toxunulmur.
     ══════════════════════════════════════════════════════════ */
  var _hookedEl = null;
  ...
  function patchHtml(html) {
    try {
      var c = cfg();
      if (!c.on || !onDashRoute()) return html;
      if (isUser() && c.simpleDash) return renderUserDash(false);
      if (isUser() || (isAdmin() && c.greetAdminToo)) return applyGreeting(html);
    } catch (e) {}
    return html;
  }
  ...
  function hookMain() {
    ...
    if (_hookedEl === main && main.__jumHook) return true;
    var desc = null;
    try {
      desc = Object.getOwnPropertyDescriptor(Element.prototype, 'innerHTML');
    } catch (e) {}
    if (!desc || !desc.set || !desc.get) return false;
    try {
      Object.defineProperty(main, 'innerHTML', {
        configurable: true,
        get: function () { return desc.get.call(main); },
        set: function (v) { desc.set.call(main, patchHtml(String(v))); }
      });
      main.__jumHook = true;
      _hookedEl = main;
      return true;
  ...
  /* ══════════════════════════════════════════════════════════
     5d) AĞ SİYAHI — "heç nə görünməsin" rejimi
     ──────────────────────────────────────────────────────────
     Esqinin istəyi (08-05): işçi heç nə GÖRMƏSİN, sonra bir-bir
     icazə verilsin. Əvvəl əksinə idi — düymə görünürdü, basanda
     bloklanırdı ("məhsul əlavə et görür amma edə bilmir").

     İndi qara siyahı yox, AĞ siyahı işləyir: yalnız işçiyə
     verilmiş kartların marşrutları açıqdır. Qalan hər şey
     ekrandan gizlənir və marşrut da bağlanır.

     Təhlükəsizlik: yalnız KATALOQDA tanınan marşrutlar gizlədilir.
     Məhsul kartı (#/product/xxx) kimi naməlum linklərə toxunulmur,
  ...
  function isBlockedRoute(base) {
    for (var i = 0; i < BLOCKED.length; i++) {
      if (base === BLOCKED[i] || base.indexOf(BLOCKED[i] + '/') === 0) return true;
    }
    return false;
  }
  ...
  function sweepUi() {
    var c = cfg();
    if (!c.on || !c.whitelist || !isUser()) return;
    var allowed = allowedRouteSet();
    var els;
    try { els = document.querySelectorAll('[onclick]'); } catch (e) { return; }
    for (var i = 0; i < els.length; i++) {
      var el = els[i];
      var r = routeOfEl(el);
      if (!r) continue;
      var base = String(r).split('?')[0];

      if (allowed[base]) {                       // icazə verilib — görünsün
        if (el.__jumHid) { el.style.display = ''; el.__jumHid = false; }
        continue;
      }
      /* Yalnız tanınan modul marşrutlarını və admin ekranlarını gizlədirik */
      if (!cardByRoute(base) && !isBlockedRoute(base)) continue;
      if (!el.__jumHid) { el.style.display = 'none'; el.__jumHid = true; }
    }
  }
  ...
  function guardRoute() {
    var c = cfg();
    if (!c.on || !isUser()) return;
    var h = String(global.location.hash || '');
    var base = h.split('?')[0];

    /* AĞ SİYAHI: icazə verilməyən hər tanınan marşrut bağlıdır */
    if (c.whitelist) {
      var allowed = allowedRouteSet();
      if (allowed[base]) return;
      if (cardByRoute(base) || isBlockedRoute(base)) {
    ...
    if (!c.lockAdminRoutes) return;
    if (isBlockedRoute(base)) {
  ...
  function openCardMenu(id) {
    ...
    h.push('<div class="jum-mi" onclick="JollyUserMode.pickUser(\'' + esc(id) + '\')" ' +
           'style="background:rgba(255,255,255,.05);">' +
           '<span class="mi-ic">👤</span><span>Kimə: <b>' + esc(grantUserName()) + '</b> — dəyiş</span></div>');
  ...
  function renderAdmin() {
    ...
    h.push('<div class="dash-head"><div>' +
             ...
             '<div class="muted" style="font-size:12.5px;">v3.4 · Karta basıb saxla — sil, gizlət, icazə ver</div>' +
           '</div></div>');
    ...
    h.push('<div class="glass" style="padding:12px 14px;margin-top:14px;font-size:12.5px;line-height:1.9;">' +
             ...
             dot(!!(_hookedEl && _hookedEl.__jumHook)) + ' Yazı qarmağı (flicker-siz başlıq)<br>' +
             dot(!!domObs)    + ' Ekran müşahidəçisi (ehtiyat)<br>' +
  ...
  var API = {
    ...
    /* uzun basma menyusu */
    /* Uzun basma menyusundan işçi dəyişmək */
    pickUser: function (backToId) {
      var U = global.JollyUsers || peek('JollyUsers');
      var all = [];
      try { all = (U && U.list) ? (U.list() || []) : []; } catch (e) {}
      var cur = grantUserId();
      var h = [];
      h.push('<div class="jum-sh-h"><span class="i">👤</span>' +
             '<span class="t"><b>İcazə kimə verilsin?</b><span>seçim yadda qalır</span></span></div>');
      for (var i = 0; i < all.length; i++) {
        var u = all[i];
        if (!u || u.role === 'admin') continue;
        h.push('<div class="jum-mi" onclick="JollyUserMode.setUser(\'' + esc(u.id) + '\',\'' + esc(backToId || '') + '\')">' +
               '<span class="mi-ic">' + (cur === u.id ? '✅' : '👤') + '</span>' +
               '<span>' + esc(u.name || u.id) + '</span></div>');
      }
      h.push('<div class="jum-mi" onclick="JollyUserMode.setUser(\'*\',\'' + esc(backToId || '') + '\')">' +
             '<span class="mi-ic">' + (cur ? '🌐' : '✅') + '</span><span>Hamısı (ümumi)</span></div>');
      if (!all.length) {
        h.push('<div class="jum-empty">İşçi hesabı yoxdur — Studio → İstifadəçilər</div>');
      }
      sheet(h.join(''));
    },
    setUser: function (uid, backToId) {
      try { localStorage.setItem('jolly_grant_user', uid); } catch (e) {}
      toast('👤 İcazə hədəfi: ' + grantUserName(), 'ok');
      if (backToId) openCardMenu(backToId); else { closeSheet(); reRenderAdmin(); }
    },
    ...
    flipPerm: function (id) {
      ...
      if (setPerm(cd.perm, !now)) {
        toast((!now ? '🔓 ' : '🔒 ') + cd.label + ' — ' + grantUserName(), 'ok');
      } else {
  ...
  function boot() {
    ...
    /* ok2 (dashboard sarğısı) ŞƏRT DEYİL — DOM ehtiyatı onu əvəz edir.
       Əvvəl şərtə daxil idi və heç vaxt uğurlu olmadığı üçün boot
       40 dəfə × 250 ms = 10 saniyə boş yerə təkrarlanırdı. Açılışın
       549 ms → 4208 ms qalxmasının səbəblərindən biri budur. */
    var coreOk = ok1 && ok3 && ok4;
    ++tries;
    if (coreOk) {
      syncModulePerms();
      console.log('[UserMode v3.4] hazırdır — kataloq:', catalog().length, 'kart');
    ...
    if (tries > 40) {
      ...
      if (miss.length) toast('👥 v3.4 — qurulmadı: ' + miss.join(', '), 'error');
      guardRoute();
      startDomFix();
      return;
    }
  ...
  /* Müşahidəçi BODY-yə bağlanır: #main elementi tam əvəzlənsə belə
     müşahidə ölmür (əvvəl #main-ə bağlanırdı və element dəyişəndə
     müşahidəçi sakitcə ölürdü — flicker-in ikinci səbəbi). */
  ...
  function startDomFix() {
    ...
    domT = setInterval(function () {
      ...
      hookMain();                                       // #main əvəzlənibsə yenidən tut
      if (!_wrapOk) _wrapOk = installDashboardWrap();   // gec gəlsə də tutaq
      domFixSafe();
    }, 1200);
    global.addEventListener('hashchange', function () {
      setTimeout(function () { hookMain(); attachObserver(); domFixSafe(); }, 60);
    });
  }
...
/* ══════════════════════════════════════════════════════════
   ★ 2026-08-19 — KASSİR REJİMİNDƏ ÇƏKİLİR
   ──────────────────────────────────────────────────────────
   Bu fayl `index.html`-dən çıxarıldı, amma Service Worker köhnə
   nüsxəni keşdən verə bilər. Belə halda o, yenə öz iş masasını
   çəkirdi və "🔒 Bu bölməyə icazən yoxdur" yazısını atırdı.

   Yoxlama YÜKLƏNMƏ ANINDA yox, HƏR ÇAĞIRIŞDA aparılır — çünki
   `jolly-cc.js` bu fayldan SONRA yüklənir və o anda `JollyCC`
   hələ mövcud olmur.
   ══════════════════════════════════════════════════════════ */
(function (g) {
  function ccOwns() {
    try {
      var CC = g.JollyCC;
      return !!(CC && typeof CC.isWorker === 'function' && CC.isWorker());
    } catch (e) { return false; }
  }

  function wrap(obj) {
    if (!obj || obj.__ccWrapped) return;
    obj.__ccWrapped = true;
    ['render', 'sweep', 'apply', 'renderDash', 'guard', 'init', 'boot']
      .forEach(function (k) {
        if (typeof obj[k] !== 'function') return;
        var orig = obj[k];
        obj[k] = function () {
          if (ccOwns()) return (k === 'render' || k === 'renderDash') ? '' : undefined;
          return orig.apply(obj, arguments);
        };
      });
  }

  /* Obyekt bu fayl işə düşəndən sonra yaranır — bir neçə dəfə yoxlayırıq */
  var n = 0;
  var t = setInterval(function () {
    if (g.JollyUserMode) { wrap(g.JollyUserMode); clearInterval(t); }
    if (++n > 40) clearInterval(t);
  }, 200);
  if (g.JollyUserMode) wrap(g.JollyUserMode);
})(typeof window !== 'undefined' ? window : this);
