...
(function (global) {
  ...
  function renderAdmin() {
    ...
    h.push('<div class="dash-head"><div>' +
             ...
             '<div class="muted" style="font-size:12.5px;">v2.5 · Karta basıb saxla — sil, gizlət, icazə ver</div>' +
           '</div></div>');
  ...
  function boot() {
    ...
    if (coreOk && ok2) {
      syncModulePerms();
      console.log('[UserMode v2.5] hazırdır — kataloq:', catalog().length, 'kart');
      toast('👥 İşçi Rejimi v2.5 — hamısı quruldu');
      guardRoute();
      schedulePermSync();
      startDomFix();
      return;
    ...
    if (tries > 40) {
      ...
      if (!ok2) miss.push('dashboard sarğısı');
      if (!ok3) miss.push('icazə');
      if (!ok4) miss.push('modul');
      toast('👥 v2.5 — qurulmadı: ' + (miss.join(', ') || 'yoxdur') + ' (DOM ehtiyatı işləyir)');
      guardRoute();
      startDomFix();
      return;
    }
