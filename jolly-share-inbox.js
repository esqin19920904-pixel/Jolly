/* ==========================================================================
   JOLLY — jolly-share-inbox.js                v3.0.0   (2026-07-30)
   --------------------------------------------------------------------------
...
(function (global) {
  ...
  function run() {
    ...
    return latest().then(function (rec) {
      if (!rec) { state.rec = null; paint(); return; }
      state.rec = rec;
      try {
  ...
  function view() {
    ...
    if (!state.rec) {
      ...
      h.push('<div class="btn" data-warm="1" id="jsi-warmbtn">⚡ Barmaq izlərini hazırla</div>');
      if (state.warm) {
    ...
    h.push('<div class="btn" data-pick="1">📁 Qalereyadan şəkil seç</div>');
    h.push('<div class="btn" data-warm="1">⚡ Barmaq izlərini hazırla</div>');
    if (state.warm) {
      h.push('<div class="stats" id="jsi-warm">⚡ ' + state.warm.done + ' / ' + state.warm.total +
             ' · yeni: ' + state.warm.made + ' · xəta: ' + state.warm.errors +
             (state.warm.running ? '' : ' · bitdi') + '</div>');
  ...
  var Inbox = {
    ...
    health: function () {
      return latest().then(function (rec) {
        ...
        var vs = lex('JollyVisualSearch');
        if (vs && vs.version !== '3.0.0') problems.push('visual-search.js köhnədir (v3.0.0 lazımdır)');
