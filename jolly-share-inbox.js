/* ==========================================================================
   JOLLY — jolly-share-inbox.js                v5.1.0   (2026-07-30)
   --------------------------------------------------------------------------
...
(function (global) {
  ...
  function lsearch(q) {
    // v5.0: axtarış məntiqi artıq QLOBAL mühərrikdədir (JollySearch)
    try { return Search.products(q, { limit: 30 }); }
    catch (e) { return []; }
  }
  ...
  global.JollyDashboard = { refresh: function () { try { dsInject(); } catch (e) {} },
                            off: dsOff,
  ...
  global.JollyLiveSearch = { attach: function () { try { lsAttach(); } catch (e) {} },
                             detach: lsDetach, rows: function () { return LS.rows; } };
  ...
  /* ======================================================================
     ⚡ CANLI AXTARIŞ QOŞMASI  (v4.8)
     Mövcud axtarış formalarına toxunmadan, onların xanasına canlı
     nəticə paneli qoşur. Açıb-bağlamaq ⚡ çipi ilə, ekran üzrə yadda.
     ====================================================================== */

  var LS = { input: null, panel: null, timer: null, hash: '', rows: [] };

  /* v5.1: canlı axtarış artıq HƏR YERDƏ STANDART AÇIQDIR.
     Yaddaşda yalnız SÖNDÜRÜLƏN ekranlar saxlanılır (əvvəl əksi idi —
     Esqin hər ekranda əl ilə açmalı olurdu). */
  ...
  /* ekrandaki axtarış xanasını tap */
  function lsFindInput() {
    var root = document.getElementById('main') || document.body;
    var list = root.querySelectorAll('input');
    for (var i = 0; i < list.length; i++) {
      var el = list[i];
      if (el.id === 'jss-input' || el.id === 'jlk-q') continue;
      var type = (el.getAttribute('type') || 'text').toLowerCase();
      if (type !== 'text' && type !== 'search') continue;
      var ph = (el.getAttribute('placeholder') || '') + ' ' + (el.getAttribute('name') || '') + ' ' + (el.id || '');
      if (/axtar|search|ad |kod|barkod|ara/i.test(ph) || type === 'search') return el;
    }
    return null;
  }
  ...
  function lsPaint() {
    if (!LS.panel) return;
    var body = LS.panel.querySelector ? LS.panel.querySelector('.ls-body') : null;
    if (body) body.innerHTML = lsRowsHTML();
    var c = LS.panel.querySelector ? LS.panel.querySelector('.ls-count') : null;
    if (c) c.textContent = LS.rows.length ? (LS.rows.length + ' nəticə') : '';
  }

  function lsRun(q) {
    if (!q || q.trim().length < 2) { LS.rows = []; lsPaint(); return; }
  ...
  function lsAttach() {
    ...
    if (lsMine(hash)) return lsDetach();

    var inp = lsFindInput();
    if (!inp) return lsDetach();
    if (LS.input === inp && document.getElementById('ls-chip')) return;

    lsDetach();
    LS.input = inp; LS.hash = hash;
    ...
    chip.id = 'ls-chip';
    chip.className = 'ls-chip' + (lsOnFor(hash) ? ' on' : '');
    chip.textContent = lsOnFor(hash) ? '⚡ canlı axtarış açıq' : '⚡ canlı axtarış sönülü';
    chip.addEventListener('click', function (e) {
      e.stopPropagation();
      var on = !lsOnFor(hash);
      lsSet(hash, on);
      chip.className = 'ls-chip' + (on ? ' on' : '');
      chip.textContent = on ? '⚡ canlı axtarış açıq' : '⚡ canlı axtarış sönülü';
      if (on) { lsRun(LS.input.value || ''); }
      else { LS.rows = []; lsPaint(); }
      if (LS.panel) LS.panel.style.display = on ? '' : 'none';
    });

    var panel = document.createElement('div');
    panel.className = 'ls-panel';
    panel.style.display = lsOnFor(hash) ? '' : 'none';
    panel.innerHTML = '<div class="ls-head"><span>⚡ canlı nəticə</span><span class="ls-count"></span></div>' +
    ...
    try {
      if (inp.nextSibling) host.insertBefore(chip, inp.nextSibling); else host.appendChild(chip);
      if (chip.nextSibling) host.insertBefore(panel, chip.nextSibling); else host.appendChild(panel);
    } catch (e) { return; }

    LS.panel = panel;

    inp.addEventListener('input', function () {
      if (!lsOnFor(LS.hash)) return;
      if (LS.timer) clearTimeout(LS.timer);
      LS.timer = setTimeout(function () { lsRun(inp.value || ''); }, 170);
    });

    if (lsOnFor(hash) && inp.value) lsRun(inp.value);
  }

  function lsDetach() {
    try {
      var c = document.getElementById('ls-chip');
      if (c) c.remove();
      if (LS.panel) LS.panel.remove();
    } catch (e) {}
    LS.input = null; LS.panel = null; LS.rows = [];
  }

  var LSCSS = [
    '.ls-chip{display:inline-block;margin-top:8px;padding:8px 13px;border-radius:999px;font-size:12px;',
    'font-weight:700;cursor:pointer;border:1px solid rgba(255,255,255,.14);',
    'background:rgba(255,255,255,.05);color:#cfd3e0}',
    '.ls-chip.on{border-color:rgba(245,196,81,.5);background:rgba(245,196,81,.14);color:#f7d98a}',
    '.ls-panel{margin-top:9px;border-radius:15px;overflow:hidden;',
    'background:rgba(12,14,22,.85);border:1px solid rgba(255,255,255,.1)}',
    '.ls-head{display:flex;justify-content:space-between;padding:9px 12px;font-size:11px;opacity:.55;',
    'letter-spacing:.6px;text-transform:uppercase}',
    '.ls-row{display:flex;align-items:center;gap:8px;padding:10px 12px;',
    'border-top:1px solid rgba(255,255,255,.06)}',
    '.ls-m{flex:1;min-width:0;cursor:pointer}',
    '.ls-nm{font-size:13.5px;font-weight:600;color:#e8e8f0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
    '.ls-cd{font-family:ui-monospace,monospace;font-size:12.5px;color:#f7d98a;margin-top:2px;opacity:.9}',
    '.ls-b{flex:none;padding:9px 11px;border-radius:10px;cursor:pointer;font-size:13px;',
    'border:1px solid rgba(245,196,81,.45);background:rgba(245,196,81,.14);color:#f7d98a}',
    '.ls-i{flex:none;padding:9px 11px;border-radius:10px;cursor:pointer;font-size:13px;',
    'border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);color:#e8e8f0}',
    '.ls-empty{padding:14px 12px;font-size:12.5px;opacity:.5;text-align:center}'
  ].join('');
  ...
  function lsStart() {
    ...
    function schedule() {
      ...
      tries = setInterval(function () {
        n++;
        try { lsAttach(); } catch (e) {}
        if (document.getElementById('ls-chip') || n > 12) clearInterval(tries);
      }, 300);
    }
    global.addEventListener('hashchange', function () { lsDetach(); schedule(); });
    schedule();
  }
  ...
  function dsPaint() {
    ...
    wrap.innerHTML = dsSearchHTML() + dsPinsHTML() + dsPanelsHTML();
  ...
  var DSCSS = [
    ...
    '#jds-wrap .jds-qrow{display:flex;gap:8px;align-items:center}',
  ...
  function dsStart() {
    ...
    global.addEventListener('hashchange', function () { dsOff(); schedule(); });
    schedule();
    try {
      var host = document.getElementById('main') || document.body;
