/* ==========================================================================
   JOLLY — jolly-share-inbox.js                v5.0.0   (2026-07-30)
   --------------------------------------------------------------------------
...
(function (global) {
  ...
  function lsKey() { return 'jolly_live_search'; }
  ...
  function lsSet(hash, on) {
    try {
      var m = JSON.parse(localStorage.getItem(lsKey()) || '{}');
      if (on) m[hash] = 1; else delete m[hash];
  ...
  function lsAttach() {
    ...
    chip.textContent = lsOnFor(hash) ? '⚡ canlı axtarış açıq' : '⚡ canlı axtarış';
    chip.addEventListener('click', function (e) {
      e.stopPropagation();
      var on = !lsOnFor(hash);
      lsSet(hash, on);
      chip.className = 'ls-chip' + (on ? ' on' : '');
      chip.textContent = on ? '⚡ canlı axtarış açıq' : '⚡ canlı axtarış';
