/* ==========================================================================
   JOLLY — jolly-share-inbox.js                v3.2.0   (2026-07-30)
   --------------------------------------------------------------------------
...
(function (global) {
  ...
  function barcodeSVG(bits, rotated) {
    ...
    var vb = '0 0 ' + total + ' ' + h;
    var style = rotated
      ? 'width:' + 'auto' + ';height:78vw;transform:rotate(90deg);'
      : 'width:100%;height:150px;';
    return '<svg viewBox="' + vb + '" preserveAspectRatio="none" style="' + style +
           'display:block;background:#fff" xmlns="http://www.w3.org/2000/svg">' +
           out.join('') + '</svg>';
  }
  ...
  function showBarcode(code, name) {
    ...
    if (enc.ok) {
      h.push('<div class="bcimg" id="jsi-bcimg">' + barcodeSVG(enc.bits, false) + '</div>');
      ...
      h.push('<div class="bcb" data-bcrot="1">⟲ Çevir</div>');
      h.push('<div class="bcb" data-copy="' + esc(enc.digits) + '">📋 Kopyala</div>');
      h.push('</div>');
      h.push('<div class="bctip">Ekranın işığını maksimuma qaldır və skaneri 10-15 sm məsafədən tut.</div>');
    } else {
    ...
    box.addEventListener('click', function (e) {
      var t = e.target;
      if (t.closest && t.closest('[data-bcclose]')) { keepAwake(false); box.remove(); return; }
  ...
  var CSS = [
    ...
    'border:1px solid rgba(255,255,255,.13);background:rgba(255,255,255,.05)}',
    '#jsi .bcb2{flex:none;padding:10px 12px;border-radius:11px;font-size:12.5px;font-weight:700;cursor:pointer;',
    'border:1px solid rgba(120,180,255,.35);background:rgba(120,180,255,.12);color:#bcd8ff;margin-left:6px}',
    /* pasport */
    ...
    '#jsi-bc .bcimg{background:#fff;padding:10px 0}',
  ...
  function bigCard(r) {
    ...
    h.push('<div class="bb" data-bar="' + esc(code) + '" data-bname="' + esc(r.product.name || '') + '">📊 Barkodu ekranda göstər</div>');
    h.push('<div class="bb" data-open="' + esc(r.product.id) + '">📄 Malın pasportu</div>');
    h.push('</div></div>');
    return h.join('');
  }
  ...
  function hitRow(r, i) {
    ...
    if (code) h.push('<div class="bcb2" data-bar="' + esc(code) + '" data-bname="' + esc(p.name || '') + '">📊</div>');
    h.push('</div>');
