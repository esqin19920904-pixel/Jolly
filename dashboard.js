...
const JollyDashboard = (() => {
  ...
  const BADGES = {
    ...
    markedCount() { return (typeof JollyDB.getMarkedForDeletion === 'function') ? JollyDB.getMarkedForDeletion().length : 0; },
    backupDue() {
  ...
  /* v5.8 (2026-07-30) — hər kartın sabit id-si var ki, Dashboard Studio
     onu göstərib-gizlədə bilsin. Əvvəl bu siyahı sabit idi və Dashboard
     Studio-dakı seçimlər İŞ MASASINA HEÇ TƏSİR ETMİRDİ. */
  const MORE = [
    { id: 'm_gallery',   icon: 'image', label: 'Qalereya', neon: 'teal', route: '#/dashboard/gallery' },
    { id: 'm_recent',    icon: 'clock', label: 'Sonuncular', neon: 'purple', route: '#/dashboard/recent' },
    { id: 'm_edited',    icon: 'clock', label: 'Son dəyişdirilən', neon: 'teal', route: '#/dashboard/recent-edited' },
    { id: 'm_drafts',    icon: 'inbox', label: 'Gələnlər', neon: 'red', route: '#/drafts', perm: 'products.create' },
    { id: 'm_capture',   icon: 'camera', label: 'Şəkil çək', sub: 'Yalnız çəkim', neon: 'gold', action: "JollyDashboard.employeeCapture()", perm: 'products.capture' },
    { id: 'm_brain',     icon: 'brain', label: 'AI Brain', neon: 'purple', route: '#/brain' },
    { id: 'm_doctor',    icon: 'barcode', label: 'Barkod Doctor', neon: 'blue', route: '#/data-doctor' },
    { id: 'm_backup',    icon: 'cloud', label: 'Backup', neon: 'teal', route: '#/studios/data' },
    { id: 'm_map',       icon: 'map', label: 'Rəf xəritəsi', neon: 'blue', route: '#/store-map' },
    { id: 'm_fav',       icon: 'star', label: 'Seçilmişlər', neon: 'gold', route: '#/dashboard/favorites' },
    { id: 'm_receiving', icon: 'inbox', label: 'Qəbul Studio', neon: 'green', route: '#/receiving' },
    { id: 'm_scanrec',   icon: 'scancenter', label: 'Skan ilə Qəbul', neon: 'gold', route: '#/scan-receiving' },
    { id: 'm_expiring',  icon: 'shield', label: 'SKT bitir', neon: 'red', route: '#/products?filter=expiring' },
    { id: 'm_photofind', icon: 'camera', label: 'Şəkillə axtar', sub: 'Bazadan tap', neon: 'blue', action: "JollyProducts.photoSearch()" },
    { id: 'm_groups',    icon: 'box', label: 'Qruplar', neon: 'gold', route: '#/studios/admin/groups', perm: 'groups.manage' },
    { id: 'm_marked',    icon: 'trash', label: '✕ İşarəli Məhsullar', neon: 'red', route: '#/marked-for-deletion', perm: 'products.admin', badgeFn: 'markedCount' },
  ];

  /* JOLLY alətləri — Dashboard Studio-dan əlavə olunur */
  const TOOLS = [
    { id: 't_kassa',  emoji: '🧾', label: 'Kassa Barkodu', neon: 'gold',   route: '#/barcode-view' },
    { id: 't_share',  emoji: '📥', label: 'Şəkillə axtarış', neon: 'purple', route: '#/share-inbox' },
    { id: 't_fix',    emoji: '🔧', label: 'Barkod Düzəldici', neon: 'red',  route: '#/barcode-fix' },
  ];

  /* Bütün qeydiyyatdan keçmiş modullar (61 ekran) da seçimə çıxır */
  function moduleCards() {
    try {
      var MR = window.ModuleRegistry || (new Function('try{return ModuleRegistry}catch(e){return null}'))();
      if (!MR) return [];
      var list = (typeof MR._all === 'function') ? MR._all() : (typeof MR.list === 'function' ? MR.list() : []);
      return (list || []).filter(m => m && m.route).map(m => ({
        id: 'mod_' + String(m.route).replace(/[^a-z0-9]+/gi, '_'),
        emoji: m.icon || '▪️', label: m.name || m.route, neon: 'blue', route: m.route, perm: m.perm
      }));
    } catch (e) { return []; }
  }
  ...
  function allCards() {
    const seen = {};
    return MORE.concat(TOOLS, moduleCards()).filter(c => {
  ...
  function render() {
    ...
    const moreCard = (c) => `
      <div class="more-card neon-${c.neon}" ${c.perm ? `data-perm="${c.perm}"` : ''} onclick="${c.action ? c.action : `JollyRouter.go('${c.route}')`}">
        <div class="mc-icon">${c.emoji ? `<span style="font-size:21px;line-height:1">${c.emoji}</span>` : JollyIcons.get(c.icon)}</div>
      ...
      <div class="storeos">
        ...
        <div class="more-grid gold-more">
          ${(() => {
            const cfg = getConfig();
            const all = allCards();
            return cfg.more.map(id => all.find(c => c.id === id)).filter(Boolean).map(moreCard).join('');
          })()}
        </div>
  ...
  function renderGallery() {
    ...
    _galleryGroupFilter = ''; _galleryQuery = ''; _gallerySelectMode = false; _gallerySelectedIds = new Set();
    setTimeout(() => renderGalleryBody(products, drafts), 0);
    return `
      ${backBtn()}
      <div class="row between" style="margin-bottom:10px;">
        <h2 style="font-family:var(--font-display);margin:0;font-size:19px;">🖼️ Şəkil qalereyası</h2>
        <span id="galSelectToggle" style="font-size:12.5px;color:var(--accent-1);cursor:pointer;" onclick="JollyDashboard.toggleGallerySelectMode()">☑️ Seç</span>
      </div>
      <input id="galSearch" placeholder="Qalereyada axtar (ad, kod...)" style="width:100%;padding:11px 14px;border-radius:10px;background:rgba(255,255,255,0.04);border:1px solid var(--border-soft);color:var(--text-hi);margin-bottom:12px;" oninput="JollyDashboard.searchGallery(this.value)">
      ...
      <div id="galBulkBar"></div>
      <div id="galleryBody"></div>
    `;
  }

  function _galleryCurrentData() {
    return {
      products: JollyDB.Products.all().filter(p => p.images && p.images.length),
      drafts: JollyDB.Drafts.all().filter(d => d.images && d.images.length),
    };
  }
  ...
  function filterGalleryByGroup(group) {
    ...
    const d = _galleryCurrentData();
    renderGalleryBody(d.products, d.drafts);
  }

  function searchGallery(q) {
    _galleryQuery = (q || '').toLowerCase().trim();
    const d = _galleryCurrentData();
    renderGalleryBody(d.products, d.drafts);
  }

  function toggleGallerySelectMode() {
    _gallerySelectMode = !_gallerySelectMode;
    _gallerySelectedIds.clear();
    const toggle = document.getElementById('galSelectToggle');
    if (toggle) toggle.textContent = _gallerySelectMode ? '✕ Seçimi bitir' : '☑️ Seç';
    const d = _galleryCurrentData();
    renderGalleryBody(d.products, d.drafts);
  }

  function toggleGallerySelect(id) {
    if (_gallerySelectedIds.has(id)) _gallerySelectedIds.delete(id); else _gallerySelectedIds.add(id);
    const d = _galleryCurrentData();
    renderGalleryBody(d.products, d.drafts);
  }

  function _renderGalBulkBar() {
    const bar = document.getElementById('galBulkBar');
    if (!bar) return;
    if (!_gallerySelectMode || !_gallerySelectedIds.size) { bar.innerHTML = ''; return; }
    bar.innerHTML = `
      <div class="glass" style="padding:10px 14px;margin-bottom:12px;display:flex;align-items:center;gap:10px;">
        <span style="flex:1;font-size:13px;">${_gallerySelectedIds.size} seçildi</span>
        <button class="btn btn-primary btn-sm" onclick="JollyDashboard.bulkMoveGallerySelection()">📦 Köçür</button>
        <button class="btn btn-ghost btn-sm" style="color:var(--accent-danger);" onclick="JollyDashboard.bulkDeleteGallerySelection()">🗑️ Sil</button>
      </div>
    `;
  }

  function bulkDeleteGallerySelection() {
    if (!_gallerySelectedIds.size) return;
    if (!confirm(`${_gallerySelectedIds.size} məhsul silinsin? (Silinənlər səbətinə düşəcək)`)) return;
    _gallerySelectedIds.forEach(id => JollyDB.Trash.moveToTrash(id));
    Toast.success(`${_gallerySelectedIds.size} məhsul silindi`);
    _gallerySelectedIds.clear();
    _gallerySelectMode = false;
    const toggle = document.getElementById('galSelectToggle');
    if (toggle) toggle.textContent = '☑️ Seç';
    const d = _galleryCurrentData();
    renderGalleryBody(d.products, d.drafts);
  }

  function bulkMoveGallerySelection() {
    if (!_gallerySelectedIds.size) return;
    const groups = JollyDB.Groups.all();
    if (!groups.length) { Toast.error('Əvvəlcə bir qrup yarat'); return; }
    const names = groups.map((g, i) => `${i + 1}. ${g.name}`).join('\n');
    const choice = prompt(`Seçilmiş ${_gallerySelectedIds.size} məhsul hansı qrupa köçürülsün?\n\n${names}\n\nRəqəmi yaz:`);
    if (choice === null) return;
    const idx = parseInt(choice, 10) - 1;
    const g = groups[idx];
    if (!g) { Toast.error('Düzgün rəqəm yazılmadı'); return; }
    _gallerySelectedIds.forEach(id => JollyDB.Products.update(id, { group: g.name }));
    Toast.success(`${_gallerySelectedIds.size} məhsul "${g.name}" qrupuna köçürüldü`);
    _gallerySelectedIds.clear();
    _gallerySelectMode = false;
    const toggle = document.getElementById('galSelectToggle');
    if (toggle) toggle.textContent = '☑️ Seç';
  ...
  function renderGalleryBody(products, drafts) {
    ...
    if (!el) return;
    let filtered = _galleryGroupFilter ? products.filter(p => p.group === _galleryGroupFilter) : products;
    if (_galleryQuery) {
      filtered = filtered.filter(p => [p.name, p.mainCode, p.extraCodeValue].filter(Boolean).join(' ').toLowerCase().includes(_galleryQuery));
    }
    const cells = [];
    filtered.forEach(p => p.images.forEach((img, i) => cells.push({ img, name: p.name, id: p.id, route: `#/product/${p.id}`, images: p.images, idx: i })));
    if (!_galleryGroupFilter && !_galleryQuery) drafts.forEach(d => d.images.forEach((img, i) => cells.push({ img, name: (d.name || 'Qaralama'), route: `#/product/new?draft=${d.id}`, images: d.images, idx: i, draft: true })));
    el.innerHTML = cells.length ? `<div class="gallery-grid">
        ${cells.map((c, i) => {
          const selected = c.id && _gallerySelectedIds.has(c.id);
          const cellClick = (_gallerySelectMode && c.id)
            ? `JollyDashboard.toggleGallerySelect('${c.id}')`
            : `JollyDashboard.openGalleryImage(${JollyProducts.escapeHtml(JSON.stringify(c.images))}, ${c.idx})`;
          return `
          <div class="gallery-cell anim-pop peekable" style="position:relative;${selected ? 'outline:2px solid var(--accent-1,#7c8aff);' : ''}" data-pki='${JollyProducts.escapeHtml(JSON.stringify(c.images))}' data-pkx="${c.idx}" onclick='${cellClick}' oncontextmenu="event.preventDefault();JollyRouter.go('${c.route}')">
            ${_gallerySelectMode && c.id ? `<div style="position:absolute;top:5px;left:5px;z-index:3;width:22px;height:22px;border-radius:50%;background:${selected ? 'var(--accent-1,#7c8aff)' : 'rgba(0,0,0,0.5)'};display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;border:1px solid rgba(255,255,255,0.3);">${selected ? '✓' : ''}</div>` : ''}
  ...
  function renderStudio() {
    const cfg = getConfig();
    const all = allCards();
    const inDash = cfg.more.filter(id => all.some(c => c.id === id));
    const available = all.filter(c => !inDash.includes(c.id));
    const group = (c) => c.id.indexOf('t_') === 0 ? 'JOLLY alətləri'
                     : (c.id.indexOf('mod_') === 0 ? 'Modullar' : 'Klassik kartlar');
    const groups = {};
    ...
    return `
      ...
      <p class="muted" style="font-size:12.5px;margin:0 0 14px;">
        İş masasında nə görünsün — sən seçirsən. Gizlətmək silmək deyil, aşağıdan geri qaytarırsan.</p>
      ...
      <div class="glass" style="padding:12px 14px;margin-top:14px;">
        <div class="row between">
          <span class="muted" style="font-size:12px;">Hər şeyi standart görünüşə qaytar</span>
          <button class="btn btn-sm" onclick="JollyDashboard.resetLayout()">↩ Standart</button>
        </div>
      </div>
    `;
  }
  ...
  function addItem(id) {
    const cfg = getConfig();
    if (!cfg.more.includes(id)) { cfg.more.push(id); setConfig(cfg); }
    Toast.success('İş masasına əlavə olundu');
    JollyRouter.go('#/dashboard/studio');
  }
  function removeItem(id) {
    const cfg = getConfig();
    cfg.more = cfg.more.filter(x => x !== id);
    setConfig(cfg);
    Toast.info('Gizlədildi (aşağıdan geri qaytara bilərsən)');
    JollyRouter.go('#/dashboard/studio');
  }
  /* JOLLY funksiyalarının açarı — jolly-share-inbox.js bu açarları oxuyur */
  function toggleTool(key, def) {
    const raw = localStorage.getItem(key);
    const on = raw === null ? (def !== false) : raw === '1';
    localStorage.setItem(key, on ? '0' : '1');
    Toast.info(on ? 'Söndürüldü' : 'Açıldı');
    JollyRouter.go('#/dashboard/studio');
  }
  function resetLayout() {
    const cfg = getConfig();
    cfg.more = MORE.map(c => c.id);
    setConfig(cfg);
    ['jolly_addon_live', 'jolly_addon_studio', 'jolly_addon_dash', 'jolly_bc_auto'].forEach(k => localStorage.removeItem(k));
    Toast.success('Standart görünüş qaytarıldı');
    JollyRouter.go('#/dashboard/studio');
  }
