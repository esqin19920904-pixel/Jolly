...
class AdminUI {
  ...
  _draw() {
    ...
    this._el.innerHTML = `
      ...
      <div class="glass" style="padding:4px 14px;margin-bottom:10px;">
        ${_getAuditLog().slice(0,30).map(l=>`
          <div class="list-row" style="padding:8px 4px;flex-direction:column;align-items:flex-start;gap:2px;">
            <span style="font-size:12px;">${_auditActionText(l)}</span>
            <span style="font-size:10px;color:var(--text-3,#64748b);">👤 ${l.actorName || l.actor} · ${l.date}</span>
          </div>
  ...
  _save() {
    ...
    const before = this.os.store.load().userOverrides[this._selectedUserId] || {};
    ...
    // Hər dəyişən icazəni ayrıca, oxuna bilən şəkildə jurnala yaz —
    // "count: 3" əvəzinə "Zülfiqara Barkod skan et icazəsi VERİLDİ" kimi.
    const labelOf = (k) => { const p = this.os.reg.allPerms().find(x => x.key === k); return p ? p.label : k; };
    const changed = Object.keys(overrides).filter(k => before[k] !== overrides[k]);
    changed.forEach(k => {
      _audit(overrides[k] ? 'permission_granted' : 'permission_revoked', { user: uname, key: k, label: labelOf(k) });
    });
    if (!changed.length) _audit('permissions_saved', { user: uname, count: Object.keys(overrides).length, note: 'dəyişiklik yox idi' });
    this.os.syncUI();
  ...
  _clearAudit() {
    JollyDB.write(JollyDB.KEYS.permAudit, []);
...
function _audit(action, details) {
  try {
    ...
    const sess = JSON.parse(sessionStorage.getItem('jolly_sec_session')||'null');
    logs.unshift({ id: 'paud_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2,7), action, details, actor: sess?.role||'?', actorName: sess?.userName || (sess?.role==='admin' ? 'Admin' : '?'), ts: Date.now(), date: new Date().toLocaleString('az-AZ') });
    JollyDB.write(JollyDB.KEYS.permAudit, logs.slice(0,500));
  } catch(e) {}
}
function _getAuditLog() {
  try { return JollyDB.read(JollyDB.KEYS.permAudit, []); } catch(e) { return []; }
}
function _auditActionText(l) {
  const d = l.details || {};
  if (l.action === 'permission_granted') return `${d.user} üçün "${d.label}" icazəsi VERİLDİ`;
  if (l.action === 'permission_revoked') return `${d.user} üçün "${d.label}" icazəsi ALINDI`;
  if (l.action === 'permissions_saved') return `${d.user} üçün icazələr saxlanıldı${d.note ? ' (' + d.note + ')' : ''}`;
  if (l.action === 'profile_applied') return `"${d.profile}" profili tətbiq olundu${d.userId ? ' (' + d.userId + ')' : ''}`;
  if (l.action === 'custom_permission_added') return `Yeni icazə yaradıldı: "${d.label}"`;
  if (l.action === 'custom_permission_removed') return `Xüsusi icazə silindi: "${d.key}"`;
  if (l.action === 'permissions_imported') return 'İcazələr JSON-dan idxal olundu';
  return l.action;
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  POS.register({ id:'products', name:'Məhsullar', icon:'📦', permissions:[
    ...
    { key:'products.capture', label:'Yalnız şəkil çək (kataloqa girmədən)', tag:'edit', default:false },
  ]});
