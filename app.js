...
const JollyApp = (() => {
  ...
  function renderInner() {
    ...
    } else if (segments[0] === 'dashboard' && segments[1] === 'recent-edited') {
      html = JollyDashboard.renderRecentlyEdited();
    ...
    } else if (segments[0] === 'marked-for-deletion') {
      html = JollyProducts.renderMarkedForDeletionPage();
    ...
    } else if (segments[0] === 'cc' || (segments[0] === 'studios' && segments[1] === 'ai')) {
      /* ★ 2026-08-23: JOLLY AI Command Center.
         Əvvəl `#/cc` marşrutu burada TANINMIRDI — router "Səhifə
         tapılmadı" yazırdı, `jolly-cc.js` isə 300 ms sonra öz
         ekranını çəkirdi. Esqin məhz o köhnə yazını görürdü.
         İndi router özü Command Center-i çağırır. */
      html = (typeof JollyCC !== 'undefined' && JollyCC.render)
        ? JollyCC.render()
        : '<div class="empty-state"><div class="big-icon">🧠</div>' +
          '<h3>JOLLY AI yüklənir…</h3></div>';
    ...
    } else if (segments[0] === 'studios' && segments[1] === 'ai-old') {
      html = JollyStudios.renderAI();   /* köhnə AI ekranı — artıq çağırılmır */
  ...
  function silentCloudMerge() {
    ...
    JollyCloud.pull().then(payload => {
      ...
      mergeArray(JollyDB.KEYS.permAudit, payload.data.permAudit);
      mergeArray(JollyDB.KEYS.tombstones, payload.data.tombstones);
  ...
  /* DÜZƏLİŞ (2026-07-24, PIN bərpası — offline bypass boşluğu):
     Əvvəl, PIN localda yoxdursa (məs. keş/site-data təmizlənib),
     bulud yoxlaması YALNIZ `navigator.onLine === true` olanda
     cəhd edilirdi. Amma `navigator.onLine` bəzən yanlış nəticə verir,
     VƏ ƏSAS PROBLEM: cihaz HƏQİQƏTƏN offline-dırsa, bu şərt sadəcə
     keçilib PIN-siz (tam açıq) girişə icazə verirdi — kimsə şüurlu
     şəkildə internetini söndürüb keş təmizləsə, PIN-i bir daha heç vaxt
     soruşulmadan Admin kimi girə bilərdi.
     İndi: PIN localda yoxdursa VƏ Cloud aktivdirsə, `navigator.onLine`-a
     etibar ETMİRİK — HƏMİŞƏ əsl bulud sorğusu ilə yoxlayırıq (4 saniyə
     limitlə). Sorğu HƏR HANSI səbəbdən uğursuz olarsa (əsl offline,
     yavaş şəbəkə və s.), giriş açıq qoyulmur — "internetə qoşul" bloklayıcı
     ekran göstərilir. Yalnız Cloud Sync HEÇ VAXT aktiv olmayıbsa (bu halda
     yoxlanacaq mənbə yoxdur), köhnə davranış saxlanılır — bu, kod səviyyəsində
     düzələ bilməyən tək ssenaridir.
  */
  function showPinRecoveryBlockedScreen(retryFn) {
    const el = document.createElement('div');
    el.id = 'jollyPinRecoveryBlock';
    el.style.cssText = 'position:fixed;inset:0;z-index:100001;background:#05050a;display:flex;align-items:center;justify-content:center;padding:24px;';
    el.innerHTML = `
      <div style="text-align:center;max-width:320px;">
        <div style="font-size:46px;margin-bottom:14px;">🔒📡</div>
        <div style="font-family:var(--font-display,sans-serif);font-size:18px;font-weight:700;color:#fff;margin-bottom:10px;">Giriş kodu yoxlanıla bilmir</div>
        <p style="color:rgba(255,255,255,0.6);font-size:13px;line-height:1.5;margin-bottom:18px;">
          Bu cihazda PIN tapılmadı və buluda qoşulmaq mümkün olmadı — təhlükəsizlik üçün açıq giriş verilmir.
          İnternetə qoşulub yenidən sına.
        </p>
        <button id="jollyPinRetryBtn" style="padding:12px 24px;border-radius:12px;border:none;background:linear-gradient(135deg,#7c8aff,#5a67e0);color:#fff;font-weight:700;font-size:14px;cursor:pointer;">🔄 Yenidən yoxla</button>
      </div>`;
    document.body.appendChild(el);
    document.getElementById('jollyPinRetryBtn').addEventListener('click', () => {
      el.remove();
      retryFn();
    });
  }

  function fetchCloudPinWithTimeout(ms) {
    return Promise.race([
      JollyCloud.pull(),
      new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), ms || 4000)),
    ]);
  }
  ...
  function init() {
    ...
    // ── Yeni cihaz kilidi problemi / PIN bərpası ──
    const noUsers = !(window.JollyUsers && JollyUsers.list().length > 0);
    const noPin = !(JollyDB.getSettings().pin);
    const cloudReady = typeof JollyCloud !== 'undefined' && JollyCloud.enabled();

    if ((noUsers || noPin) && cloudReady) {
      fetchCloudPinWithTimeout(4000).then(payload => {
        if (!payload || !payload.data) {
          // Bulud boşdur (əsl ilk quraşdırma) — açıq davam et
          continueBoot();
          return;
        }
        ...
        // Bulud sorğusu uğursuz oldu (offline/yavaş/xəta) VƏ localda PIN
        // yoxdur — açıq buraxmaq əvəzinə blokla, yenidən cəhd düyməsi göstər.
        if (noPin) {
          showPinRecoveryBlockedScreen(() => init());
        } else {
          continueBoot();
        }
      });
